...
export default function TrisaCompliancePage() {
  const { user } = useAuth();
  const { data, isLoading } = (trpc as any)?.["compliance"]?.["list"]?.useQuery?.() ?? { data: null, isLoading: false };
  const items: any[] = Array.isArray(data) ? data : (data ? [data] : []);

  return (
    <DashboardLayout>
      <div className="space-y-6 p-6">
        <h1 className="text-2xl font-bold">Trisa Compliance</h1>
        {isLoading ? (
