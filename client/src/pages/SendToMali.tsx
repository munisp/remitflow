...
const CORRIDOR = "ML";
const DFSP_OPTIONS = [
  { value: "ml-orange", label: "Orange Money Mali" },
  { value: "ml-moov", label: "Moov Money" },
];

export default function SendToMali() {
  const { user } = useAuth();
  const [amountNgn, setAmountNgn] = useState(50000);
  const [recipientMobile, setRecipientMobile] = useState("");
  const [recipientName, setRecipientName] = useState("");
  const [dfspId, setDfspId] = useState("ml-orange");
  ...
  const maliRate = (rates as any)?.ML;

  return (
    <div className="container max-w-4xl py-8 space-y-6">
      <div className="flex items-center gap-3">
        <span className="text-4xl">🇲🇱</span>
        <div><h1 className="text-2xl font-bold">Send Money to Mali</h1><p className="text-muted-foreground">Orange Money & Moov via Mojaloop</p></div>
        <Badge variant="secondary" className="ml-auto">XOF Corridor</Badge>
      </div>
      {maliRate && (
        <Card><CardContent className="pt-4 flex items-center gap-6">
          <div><div className="text-2xl font-bold">₦1 = {maliRate.xof_per_ngn?.toFixed(4)} XOF</div><div className="text-sm text-muted-foreground">Spread: {maliRate.spread_bps} bps</div></div>
          <Badge variant="outline" className="text-green-500 border-green-500">Live</Badge>
        </CardContent></Card>
      )}
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        <Card>
          <CardHeader><CardTitle>Send Transfer</CardTitle><CardDescription>Funds arrive in minutes</CardDescription></CardHeader>
          <CardContent>
            <form onSubmit={(e) => { e.preventDefault(); if (!user || !recipientMobile || !recipientName) { toast.error("Fill all fields"); return; } submit.mutate({ corridorCode: CORRIDOR, amountNgn, recipientMobileMoney: recipientMobile, recipientName, mojaloopDfspId: dfspId, purposeCode }); }} className="space-y-4">
              ...
              {quote && <div className="bg-muted rounded-lg p-3 text-sm space-y-1"><div className="flex justify-between"><span>Recipient gets</span><span className="font-medium text-green-500">{(quote as any).amount_xof?.toLocaleString()} XOF</span></div><div className="flex justify-between text-muted-foreground"><span>Fee</span><span>₦{(quote as any).fee_ngn?.toLocaleString() ?? "—"}</span></div></div>}
              ...
              <div className="space-y-2"><Label>Recipient Mobile</Label><div className="flex gap-2 items-center"><Phone className="h-4 w-4 text-muted-foreground" /><Input value={recipientMobile} onChange={(e) => setRecipientMobile(e.target.value)} placeholder="+223XXXXXXXX" /></div></div>
              ...
              <Button type="submit" className="w-full" disabled={submit.isPending}>{submit.isPending ? <><Loader2 className="h-4 w-4 animate-spin mr-2" />Sending...</> : <>Send to Mali <ArrowRight className="ml-2 h-4 w-4" /></>}</Button>
            </form>
          </CardContent>
        </Card>
        <Card>
          ...
          <CardContent>
            {!history || (history as any[]).length === 0 ? <p className="text-muted-foreground text-sm text-center py-8">No transfers yet</p> : (
              ...
              <TableBody>{(history as any[]).map((t) => <TableRow key={t.transferId}><TableCell>{t.recipientName}</TableCell><TableCell>₦{parseFloat(t.amountNgn).toLocaleString()}</TableCell><TableCell><Badge variant={t.status === "completed" ? "default" : t.status === "failed" ? "destructive" : "secondary"}>{t.status}</Badge></TableCell></TableRow>)}</TableBody></Table>
            )}
          </CardContent>
        </Card>
      </div>
