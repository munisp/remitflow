...
class FXAlertsScreen extends StatefulWidget {
  const FXAlertsScreen({Key? key}) : super(key: key);
  @override
...
class _FXAlertsScreenState extends State<FXAlertsScreen> {
  ...
  Future<void> _loadData() async {
    ...
    try {
      ...
      debugPrint('Error loading FX Rate Alerts: $e');
    } finally {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      appBar: AppBar(
        ...
        title: const Text('FX Rate Alerts', style: TextStyle(color: Colors.white)),
                ...
                child: ListView(children: [
                  Padding(
                    ...
                    child: ElevatedButton.icon(
                      onPressed: () => ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text('Create FX alerts from the web app'))),
                      icon: const Icon(Icons.add),
                      label: const Text('New Rate Alert'),
                      style: ElevatedButton.styleFrom(backgroundColor: const Color(0xFF6366f1), minimumSize: const Size.fromHeight(44)),
                    ),
                  ),
                  ..._alerts.where((a) => (a['pair'] ?? '').toString().toLowerCase().contains(_search.toLowerCase())).map((a) => _card(
                    title: a['pair'] ?? '\${a['fromCurrency']}/\${a['toCurrency']}',
                    subtitle: 'Target: \${a['targetRate']} (\${a['direction'] ?? 'above'})',
                    label: 'Current: \${a['currentRate'] ?? 'N/A'}',
                  )).toList(),
                  if (_alerts.isEmpty) const Center(child: Padding(padding: EdgeInsets.all(40), child: Text('No FX alerts set up.', style: TextStyle(color: Colors.grey)))),
                ]),
              )),
            ]),
    );
