...
class AdvancedFeaturesManager {
  ...
  private async saveSavingsGoals(): Promise<void> {
    ...
    await localforage.setItem('savings_goals', JSON.stringify(goals));
  }

  private async loadSavingsGoals(): Promise<void> {
    try {
      const stored = await localforage.getItem('savings_goals');
      if (stored) {
  ...
  private async saveRecurringBills(): Promise<void> {
    ...
    await localforage.setItem('recurring_bills', JSON.stringify(bills));
  }

  private async loadRecurringBills(): Promise<void> {
    try {
      const stored = await localforage.getItem('recurring_bills');
      if (stored) {
