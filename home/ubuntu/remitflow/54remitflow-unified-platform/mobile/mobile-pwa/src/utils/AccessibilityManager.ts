...
import { AccessibilityInfo, Platform } from 'react';
...
class AccessibilityManager {
  ...
  private async loadSettings(): Promise<void> {
    try {
      const saved = await localforage.getItem('accessibility_settings');
      if (saved) {
  ...
  private async saveSettings(): Promise<void> {
    try {
      await localforage.setItem('accessibility_settings', JSON.stringify(this.settings));
