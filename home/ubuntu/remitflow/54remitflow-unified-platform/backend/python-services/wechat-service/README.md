...
- ✅ Send messages via Wechat
- ✅ Receive webhooks from Wechat
- ✅ Order management
- ✅ Message tracking
...
export WECHAT_API_SECRET="your_api_secret"
export WECHAT_WEBHOOK_SECRET="your_webhook_secret"
export PORT=8097
```
...
Visit `http://localhost:8097/docs` for interactive API documentation.
...
```bash
curl -X POST http://localhost:8097/api/v1/send \
...
```bash
curl -X POST http://localhost:8097/api/v1/order \
  -H "Content-Type: application/json" \
  -d '{
    "customer_id": "CUST123",
