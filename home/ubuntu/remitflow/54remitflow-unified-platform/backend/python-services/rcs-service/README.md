...
- ✅ Send messages via Rcs
- ✅ Receive webhooks from Rcs
- ✅ Order management
- ✅ Message tracking
...
```bash
export RCS_API_KEY="your_api_key"
export RCS_API_SECRET="your_api_secret"
export RCS_WEBHOOK_SECRET="your_webhook_secret"
export PORT=8093
```
...
```bash
curl -X POST http://localhost:8093/api/v1/send \
...
```bash
curl -X POST http://localhost:8093/api/v1/order \
  -H "Content-Type: application/json" \
  -d '{
    "customer_id": "CUST123",
