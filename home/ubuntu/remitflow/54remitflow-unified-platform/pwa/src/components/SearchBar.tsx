...
import { searchService, SearchIndex } from '../services/searchService';
...
export const SearchBar: React.FC<SearchBarProps> = ({
  ...
  const handleSubmit = useCallback((searchQuery: string) => {
    if (searchQuery.trim()) {
      ...
      searchService.saveRecentSearch(searchQuery.trim(), Array.isArray(index) ? index[0] : index)
        .catch(() => {});
    }
  }, [onSearch, index]);
  ...
  const handleKeyDown = (e: React.KeyboardEvent<HTMLInputElement>) => {
    const items = [...suggestions, ...(showRecentSearches && query.length < minQueryLength ? recentSearches : [])];
            ...
            <div className="py-2 border-t border-gray-100">
              ...
              {recentSearches.map((search, idx) => (
                <button
                  ...
                  className={`w-full px-4 py-2 text-left hover:bg-gray-100 flex items-center gap-2 ${
                    selectedIndex === suggestions.length + idx ? 'bg-gray-100' : ''
                  }`}
                >
                  <svg className="w-4 h-4 text-gray-400" fill="none" stroke="currentColor" viewBox="0 0 24 24">
