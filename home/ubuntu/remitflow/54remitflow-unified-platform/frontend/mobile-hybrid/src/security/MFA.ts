...
class MFA {
  ...
  async sendSMSOTP(phoneNumber: string): Promise<boolean> {
    ...
    try {
      ...
      await AsyncStorage.setItem('sms_otp', code);
      await AsyncStorage.setItem('sms_otp_expiry', (Date.now() + 300000).toString());
  ...
  async verifySMSOTP(code: string): Promise<MFAVerification> {
    const storedCode = await AsyncStorage.getItem('sms_otp');
    const expiry = await AsyncStorage.getItem('sms_otp_expiry');
    ...
    if (isValid) {
      await AsyncStorage.removeItem('sms_otp');
      await AsyncStorage.removeItem('sms_otp_expiry');
  ...
  async sendEmailOTP(email: string): Promise<boolean> {
    ...
    try {
      ...
      await AsyncStorage.setItem('email_otp', code);
      await AsyncStorage.setItem('email_otp_expiry', (Date.now() + 600000).toString());
  ...
  async verifyEmailOTP(code: string): Promise<MFAVerification> {
    const storedCode = await AsyncStorage.getItem('email_otp');
    const expiry = await AsyncStorage.getItem('email_otp_expiry');
    ...
    if (isValid) {
      await AsyncStorage.removeItem('email_otp');
      await AsyncStorage.removeItem('email_otp_expiry');
  ...
  private async loadMFAMethods(): Promise<void> {
    const stored = await AsyncStorage.getItem('mfa_methods');
    if (stored) {
  ...
  private async saveTOTPSecret(secret: string): Promise<void> {
    await AsyncStorage.setItem('totp_secret', secret);
  }

  private async loadBackupCodes(): Promise<void> {
    const stored = await AsyncStorage.getItem('backup_codes');
    if (stored) {
  ...
  private async saveBackupCodes(codes: string[]): Promise<void> {
    await AsyncStorage.setItem('backup_codes', JSON.stringify(codes));
  }
  ...
  private async saveMFAMethods(): Promise<void> {
    ...
    await AsyncStorage.setItem('mfa_methods', JSON.stringify(methods));
  }
