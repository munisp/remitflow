...
def list_stablecoins(skip: int = 0, limit: int = 100, db: Session = Depends(get_db)):
...
def read_stablecoin(stablecoin_id: int, db: Session = Depends(get_db)):
...
def create_new_stablecoin(stablecoin: StablecoinCreate, db: Session = Depends(get_db), current_user: User = Depends(get_current_user)):
...
def update_existing_stablecoin(stablecoin_id: int, stablecoin_in: StablecoinUpdate, db: Session = Depends(get_db), current_user: User = Depends(get_current_user)):
...
def list_user_accounts(skip: int = 0, limit: int = 100, db: Session = Depends(get_db), current_user: User = Depends(get_current_user)):
...
def create_user_account(account: AccountCreate, db: Session = Depends(get_db), current_user: User = Depends(get_current_user)):
    try:
...
def update_account_deposit_borrow_rates(account_id: uuid.UUID, account_in: AccountUpdate, db: Session = Depends(get_db), current_user: User = Depends(get_current_user)):
...
def list_account_transactions(account_id: uuid.UUID, skip: int = 0, limit: int = 100, db: Session = Depends(get_db), current_user: User = Depends(get_current_user)):
...
def create_new_transaction(account_id: uuid.UUID, transaction_in: TransactionCreate, db: Session = Depends(get_db), current_user: User = Depends(get_current_user)):
    # Authorization check: Ensure the account belongs to the current user
    db_account = get_account(db, account_id)
    if not db_account or db_account.user_id != current_user.id:
        raise HTTPException(status_code=404, detail="Account not found or access denied")
