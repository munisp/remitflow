...
export default function SendToNigeriaScreen() {
  const navigation = useNavigation<any>();
  const [amount, setAmount] = React.useState('');
  const { data: rates, isLoading } = trpc.fx.getRates.useQuery({ from: 'USD', to: 'NGN' }, { onError: () => {} });
  ...
  return (
    <ScrollView style={styles.container}>
      <View style={styles.header}>
        ...
        <Text style={styles.title}>Send to Nigeria</Text>
        <Text style={styles.subtitle}>Fast, secure transfers to Nigeria</Text>
      </View>
      <View style={styles.card}>
        <Text style={styles.label}>Exchange Rate</Text>
        {isLoading ? <ActivityIndicator color="#6366f1" /> : (
          <Text style={styles.rate}>1 USD = {rate.toFixed(4)} NGN</Text>
        )}
      </View>
      <View style={styles.card}>
        ...
        />
        <Text style={styles.label}>Recipient Gets (NGN)</Text>
        <Text style={styles.converted}>{converted} Naira</Text>
      </View>
      ...
      <View style={styles.infoCard}>
        <Text style={styles.infoTitle}>Why send to Nigeria?</Text>
