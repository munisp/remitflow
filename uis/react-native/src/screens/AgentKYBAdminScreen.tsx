...
export default function AgentKYBAdminScreen() {
  const navigation = useNavigation();
  const [search, setSearch] = useState('');
  const [refreshing, setRefreshing] = useState(false);

  const { data, isLoading, error, refetch } = trpc.kyb.listPendingKYB.useQuery(undefined, {
    retry: 2,
    staleTime: 30_000,
  });
      ...
      >
        {isLoading ? (
          ...
          <View style={styles.errorContainer}>
            <Text style={styles.errorText}>Failed to load KYB Applications.</Text>
          ...
          <View style={styles.emptyContainer}>
            ...
            <Text style={styles.emptyText}>{search ? 'No results found.' : 'No KYB Applications yet.'}</Text>
          </View>
        ) : (
