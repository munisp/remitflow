...
export default function BillingEngineDashboardScreen() {
  const navigation = useNavigation();
  const [refreshing, setRefreshing] = React.useState(false);

  const { data, isLoading, error, refetch } = trpc.billing.getSummary.useQuery(undefined, {
    retry: 2,
    staleTime: 60_000,
  });
      ...
      >
        {isLoading ? (
          ...
          <View style={styles.errorContainer}>
            <Text style={styles.errorText}>Failed to load Billing Summary.</Text>
