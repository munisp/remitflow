...
export default function TalentBridgeScreen() {
  const navigation = useNavigation();
  const [search, setSearch] = useState('');
  const [refreshing, setRefreshing] = useState(false);

  const { data, isLoading, error, refetch } = trpc.talentBridge.listJobs.useQuery(undefined, {
    retry: 2,
    staleTime: 30_000,
  });
  ...
  return (
    <View style={styles.container}>
      <View style={styles.header}>
        ...
        <Text style={styles.title}>Talent Bridge</Text>
        <View style={{ width: 50 }} />
      </View>
      ...
      <ScrollView style={styles.content}
        ...
        {isLoading ? (
          ...
          <View style={styles.errorContainer}>
            <Text style={styles.errorText}>Failed to load Talent Bridge.</Text>
          ...
          <View style={styles.emptyContainer}>
            ...
            <Text style={styles.emptyText}>{search ? 'No results.' : 'No Talent Bridge yet.'}</Text>
          </View>
        ) : (
