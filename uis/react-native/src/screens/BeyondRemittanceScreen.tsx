...
export default function BeyondRemittanceScreen() {
  const navigation = useNavigation();
  const [search, setSearch] = useState('');
  const [refreshing, setRefreshing] = useState(false);

  const { data, isLoading, error, refetch } = trpc.beyondRemittance.listProducts.useQuery(undefined, {
    retry: 2,
    staleTime: 30_000,
  });
      ...
      >
        {isLoading ? (
          ...
          <View style={styles.emptyContainer}>
            ...
            <Text style={styles.emptyText}>{search ? 'No results found.' : 'No Products yet.'}</Text>
          </View>
        ) : (
