...
export default function SendToUgandaScreen() {
  const navigation = useNavigation<any>();
  const [amount, setAmount] = React.useState('');
  const { data: rates, isLoading } = trpc.fx.getRates.useQuery({ from: 'USD', to: 'UGX' }, { onError: () => {} });
  ...
  return (
    <ScrollView style={styles.container}>
      <View style={styles.header}>
        ...
        <Text style={styles.title}>Send to Uganda</Text>
        <Text style={styles.subtitle}>Fast, secure transfers to Uganda</Text>
      </View>
      <View style={styles.card}>
        <Text style={styles.label}>Exchange Rate</Text>
        {isLoading ? <ActivityIndicator color="#6366f1" /> : (
          <Text style={styles.rate}>1 USD = {rate.toFixed(4)} UGX</Text>
        )}
      </View>
      <View style={styles.card}>
        ...
        />
        <Text style={styles.label}>Recipient Gets (UGX)</Text>
      ...
      <View style={styles.infoCard}>
        <Text style={styles.infoTitle}>Why send to Uganda?</Text>
