...
export default function BulkPaymentsV2PageScreen() {
  const navigation = useNavigation<any>();
  const { user } = useAuth();
  const [refreshing, setRefreshing] = React.useState(false);
  const { data, isLoading, refetch } = (trpc as any)?.['batch']?.['getBulkPayments']?.useQuery?.() ?? { data: null, isLoading: false, refetch: () => {} };
  ...
  return (
    ...
    >
      <View style={styles.header}>
        ...
        <Text style={styles.title}>Bulk Payments V2</Text>
