...
export default function GrafanaDashboardPageScreen() {
  const navigation = useNavigation();
  const [refreshing, setRefreshing] = React.useState(false);

  const { data, isLoading, error, refetch } = trpc.monitoring.getMetrics.useQuery(undefined, {
    retry: 2,
    staleTime: 60_000,
  });
      ...
      >
        {isLoading ? (
          ...
          <View style={styles.errorContainer}>
            <Text style={styles.errorText}>Failed to load Platform Metrics.</Text>
