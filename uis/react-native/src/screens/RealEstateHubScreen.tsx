...
export default function RealEstateHubScreen() {
  const navigation = useNavigation();
  const [search, setSearch] = useState('');
  const [refreshing, setRefreshing] = useState(false);

  const { data, isLoading, error, refetch } = trpc.propertyInvest.listProperties.useQuery(undefined, {
    retry: 2,
    staleTime: 30_000,
  });
  ...
  return (
    <View style={styles.container}>
      <View style={styles.header}>
        ...
        <Text style={styles.title}>Real Estate Hub</Text>
        <View style={{ width: 50 }} />
      </View>
      ...
      <ScrollView style={styles.content}
        ...
        {isLoading ? (
          ...
          <View style={styles.errorContainer}>
            <Text style={styles.errorText}>Failed to load Real Estate Hub.</Text>
          ...
          <View style={styles.emptyContainer}>
            ...
            <Text style={styles.emptyText}>{search ? 'No results.' : 'No Real Estate Hub yet.'}</Text>
          </View>
        ) : (
