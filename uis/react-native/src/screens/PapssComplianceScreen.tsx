...
export default function PapssComplianceScreen() {
  const navigation = useNavigation();
  const [refreshing, setRefreshing] = React.useState(false);

  const { data, isLoading, error, refetch } = trpc.papss.getComplianceStatus.useQuery(undefined, {
    retry: 2,
    staleTime: 60_000,
  });
  ...
  return (
    <View style={styles.container}>
      <View style={styles.header}>
        ...
        <Text style={styles.title}>PAPSS Compliance</Text>
        <View style={{ width: 50 }} />
      </View>
      <ScrollView style={styles.content}
        ...
        {isLoading ? (
          ...
          <View style={styles.errorContainer}>
            <Text style={styles.errorText}>Failed to load PAPSS Compliance.</Text>
