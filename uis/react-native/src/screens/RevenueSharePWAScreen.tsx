...
export default function RevenueSharePWAScreen() {
  const navigation = useNavigation<any>();
  const { user } = useAuth();
  const [refreshing, setRefreshing] = React.useState(false);
  const { data, isLoading, refetch } = (trpc as any)?.['revenueShare']?.['getSummary']?.useQuery?.() ?? { data: null, isLoading: false, refetch: () => {} };
