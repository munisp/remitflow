...
export default function SanctionsScreeningPageScreen() {
  const navigation = useNavigation();
  const [search, setSearch] = useState('');
  const [refreshing, setRefreshing] = useState(false);

  const { data, isLoading, error, refetch } = trpc.compliance.listWatchlist.useQuery(undefined, {
    retry: 2,
    staleTime: 30_000,
  });
      ...
      <ScrollView style={styles.content}
        ...
        {isLoading ? (
          ...
          <View style={styles.errorContainer}>
            <Text style={styles.errorText}>Failed to load Sanctions Screening.</Text>
          ...
          <View style={styles.emptyContainer}>
            ...
            <Text style={styles.emptyText}>{search ? 'No results.' : 'No Sanctions Screening yet.'}</Text>
          </View>
        ) : (
