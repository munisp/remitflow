...
export default function ServicesHealthDashboardScreen() {
  const navigation = useNavigation<any>();
  const { user } = useAuth();
  const [refreshing, setRefreshing] = React.useState(false);
  const { data, isLoading, refetch } = (trpc as any)?.['servicesHealth']?.['getAll']?.useQuery?.() ?? { data: null, isLoading: false, refetch: () => {} };
