...
export default function SecurityAuditReportScreen() {
  const navigation = useNavigation<any>();
  const { user } = useAuth();
  const [refreshing, setRefreshing] = React.useState(false);
  const { data, isLoading, refetch } = (trpc as any)?.['security']?.['getAuditReport']?.useQuery?.() ?? { data: null, isLoading: false, refetch: () => {} };
