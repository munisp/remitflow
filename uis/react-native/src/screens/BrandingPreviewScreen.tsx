...
export default function BrandingPreviewScreen() {
  const navigation = useNavigation();
  const [refreshing, setRefreshing] = React.useState(false);

  const { data, isLoading, error, refetch } = trpc.whiteLabel.getBranding.useQuery(undefined, {
    retry: 2,
    staleTime: 60_000,
  });
  ...
  return (
    <View style={styles.container}>
      <View style={styles.header}>
        ...
        <Text style={styles.title}>Branding Config</Text>
        <View style={{ width: 50 }} />
      </View>
      ...
      >
        {isLoading ? (
          ...
          <View style={styles.errorContainer}>
            <Text style={styles.errorText}>Failed to load Branding Config.</Text>
