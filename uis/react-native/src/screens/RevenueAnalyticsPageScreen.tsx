...
export default function RevenueAnalyticsPageScreen() {
  const navigation = useNavigation<any>();
  const { user } = useAuth();
  const [refreshing, setRefreshing] = React.useState(false);
  const { data, isLoading, refetch } = (trpc as any)?.['analytics']?.['getRevenue']?.useQuery?.() ?? { data: null, isLoading: false, refetch: () => {} };
