...
export default function CbnComplianceDashboardScreen() {
  const navigation = useNavigation();
  const [refreshing, setRefreshing] = React.useState(false);

  const { data, isLoading, error, refetch } = trpc.cbnCompliance.getSummary.useQuery(undefined, {
    retry: 2,
    staleTime: 60_000,
  });
  ...
  return (
    <View style={styles.container}>
      <View style={styles.header}>
        ...
        <Text style={styles.title}>CBN Compliance</Text>
        <View style={{ width: 50 }} />
      </View>
      ...
      >
        {isLoading ? (
          ...
          <View style={styles.errorContainer}>
            <Text style={styles.errorText}>Failed to load CBN Compliance.</Text>
