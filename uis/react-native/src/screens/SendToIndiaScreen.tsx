...
export default function SendToIndiaScreen() {
  const navigation = useNavigation<any>();
  const [amount, setAmount] = React.useState('');
  const { data: rates, isLoading } = trpc.fx.getRates.useQuery({ from: 'USD', to: 'INR' }, { onError: () => {} });
  ...
  return (
    <ScrollView style={styles.container}>
      <View style={styles.header}>
        ...
        <Text style={styles.title}>Send to India</Text>
        <Text style={styles.subtitle}>Instant UPI transfers to India</Text>
      </View>
      <View style={styles.card}>
        <Text style={styles.label}>Exchange Rate</Text>
        {isLoading ? <ActivityIndicator color="#6366f1" /> : (
          <Text style={styles.rate}>1 USD = {rate.toFixed(4)} INR</Text>
        )}
      </View>
      <View style={styles.card}>
        ...
        />
        <Text style={styles.label}>Recipient Gets (INR)</Text>
      ...
      <View style={styles.infoCard}>
        <Text style={styles.infoTitle}>Why send to India via UPI?</Text>
        <Text style={styles.infoText}>• UPI instant settlement — arrives in seconds</Text>
        <Text style={styles.infoText}>• Send to any VPA (name@oksbi, name@paytm)</Text>
        <Text style={styles.infoText}>• All major banks (SBI, HDFC, ICICI, Axis)</Text>
        <Text style={styles.infoText}>• Paytm, PhonePe, Google Pay supported</Text>
        <Text style={styles.infoText}>• Mid-market rates, 0.3-0.5% margin</Text>
      </View>
    </ScrollView>
  );
}
