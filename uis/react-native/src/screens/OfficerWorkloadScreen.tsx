...
export default function OfficerWorkloadScreen() {
  ...
  const { data, isLoading, refetch } = (trpc as any)?.['compliance']?.['getOfficerWorkload']?.useQuery?.() ?? { data: null, isLoading: false, refetch: () => {} };
  const onRefresh = async () => { setRefreshing(true); await refetch?.(); setRefreshing(false); };
  const items: any[] = Array.isArray(data) ? data : (data ? [data] : []);
  return (
    <ScrollView style={styles.container} refreshControl={<RefreshControl refreshing={refreshing} onRefresh={onRefresh} />}>
      <View style={styles.header}><Text style={styles.title}>OfficerWorkload</Text></View>
