...
export default function StartupDealRoomScreen() {
  const navigation = useNavigation();
  const [search, setSearch] = useState('');
  const [refreshing, setRefreshing] = useState(false);

  const { data, isLoading, error, refetch } = trpc.diasporaInvest.listOpportunities.useQuery(undefined, {
    retry: 2,
    staleTime: 30_000,
  });
  ...
  return (
    <View style={styles.container}>
      <View style={styles.header}>
        ...
        <Text style={styles.title}>Startup Deal Room</Text>
        <View style={{ width: 50 }} />
      </View>
      <View style={styles.searchContainer}>
        <TextInput style={styles.searchInput} placeholder="Search..." placeholderTextColor="#64748b"
      ...
      <ScrollView style={styles.content}
        ...
        {isLoading ? (
          ...
          <View style={styles.errorContainer}>
            <Text style={styles.errorText}>Failed to load Startup Deal Room.</Text>
          ...
          <View style={styles.emptyContainer}>
            ...
            <Text style={styles.emptyText}>{search ? 'No results.' : 'No Startup Deal Room yet.'}</Text>
          </View>
        ) : (
          filtered.map((item: any, idx: number) => (
            <View key={item.id ?? idx} style={styles.card}>
              {Object.entries(item).filter(([k]) => !['__typename'].includes(k)).slice(0, 6).map(([k, v]) => (
                <View key={k} style={styles.row}>
                  <Text style={styles.label}>{k.replace(/([A-Z])/g, ' $1').trim()}</Text>
                  <Text style={styles.value} numberOfLines={1}>{v === null || v === undefined ? '—' : String(v)}</Text>
                </View>
              ))}
            </View>
          ))
