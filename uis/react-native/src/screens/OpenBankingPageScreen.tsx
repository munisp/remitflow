...
export default function OpenBankingPageScreen() {
  const navigation = useNavigation<any>();
  const { user } = useAuth();
  const [refreshing, setRefreshing] = React.useState(false);
  const { data, isLoading, refetch } = (trpc as any)?.['openbanking']?.['getAccounts']?.useQuery?.() ?? { data: null, isLoading: false, refetch: () => {} };
