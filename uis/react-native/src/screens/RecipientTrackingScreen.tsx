...
export default function RecipientTrackingScreen() {
  ...
  const loadData = async () => {
    try {
      setError(null);
      const res = await fetch(`${API_BASE}/api/trpc/recipient-tracking`);
  ...
  return (
    <ScrollView style={styles.container} refreshControl={<RefreshControl refreshing={refreshing} onRefresh={() => { setRefreshing(true); loadData(); }} />}>
      <Text style={styles.title}>Recipient Tracking</Text>
