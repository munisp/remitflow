...
export default function CorridorPricingAdminScreen() {
  const navigation = useNavigation<any>();
  const { user } = useAuth();
  const [refreshing, setRefreshing] = React.useState(false);
  const { data, isLoading, refetch } = (trpc as any)?.['corridorPricing']?.['list']?.useQuery?.() ?? { data: null, isLoading: false, refetch: () => {} };
  ...
  return (
    ...
    >
      <View style={styles.header}>
        ...
        <Text style={styles.title}>💱 Corridor Pricing</Text>
