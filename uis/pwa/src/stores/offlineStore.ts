      ...
      syncPendingTransactions: async () => {
        ...
        for (const txn of pendingTxns) {
          try {
            ...
            const requestBody =
              txn.type === "transfer"
                ? mapTransferToPaymentHubPayload(txn.data)
                : txn.data;
...
function getEndpointForType(type: PendingTransaction["type"]): string {
  const endpoints: Record<string, string> = {
    transfer: "/payment-hub/api/v1/transfers/initiate",
...
function mapTransferToPaymentHubPayload(data: Record<string, unknown>) {
  ...
  return {
    switch_name:
      typeof data.switchName === "string" ? data.switchName : "mojaloop",
    amount,
    currency,
    to: {
      idType: "ACCOUNT_ID",
      idValue: typeof data.recipient === "string" ? data.recipient : "",
      displayName:
    ...
    note:
      typeof data.note === "string" && data.note
        ? data.note
        : "Transfer from remittance UI",
    pin: typeof data.pin === "string" && data.pin ? data.pin : "",
  };
}
...
export const usePendingCount = () =>
  useOfflineStore((state) => state.getPendingCount());
export const useSyncInProgress = () =>
  useOfflineStore((state) => state.syncInProgress);
