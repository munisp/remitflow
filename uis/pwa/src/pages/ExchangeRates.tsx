...
const ExchangeRates: React.FC = () => {
    ...
    try {
      ...
      if (data.length > 0) {
        setRates(data.map((r: ExchangeRate) => ({
          from: r.from,
          to: r.to,
          // Backend gives NGN base -> foreign; we store as NGN per 1 foreign
          rate: r.rate,
          change: 0,
          flag: flagMap[r.to] || '',
  ...
  const getRate = (from: string, to: string): number => {
    ...
    // rates[] holds NGN-based quotes: 1 unit of `to` costs `rate` NGN.
    // Example: row for USD has rate = 1300.5, meaning 1 USD = 1300.5 NGN.

    const baseRow = rates.find(r => r.from === 'NGN' && r.to === from);
    const quoteRow = rates.find(r => r.from === 'NGN' && r.to === to);

    // We need a conversion factor so that: amount_in_to = amount_in_from * factor.

    // From NGN to foreign: amount_in_to = amount_in_from / rate_to
    if (from === 'NGN' && quoteRow && quoteRow.rate > 0) {
      return 1 / quoteRow.rate;
    }

    // From foreign to NGN: amount_in_to = amount_in_from * rate_from
    if (to === 'NGN' && baseRow) {
      return baseRow.rate;
    }

    // Foreign to foreign: go via NGN
    if (baseRow && quoteRow && baseRow.rate > 0) {
      // 1 from = baseRow.rate NGN; 1 to = quoteRow.rate NGN
      // so from -> to = baseRow.rate / quoteRow.rate
      return baseRow.rate / quoteRow.rate;
    }
            ...
            <tbody>
              {rates.map((rate) => (
                <tr key={rate.to} className="border-b border-slate-50 hover:bg-slate-50 transition-colors">
                  ...
                  <td className="py-3.5 px-4 text-right font-mono text-sm font-semibold text-slate-900">{rate.rate > 0 ? rate.rate.toFixed(2) : '0.00'}</td>
      ...
      <div className="bg-white rounded-2xl border border-slate-100 p-5">
        ...
        <div className="flex flex-col md:flex-row gap-3">
          ...
          <input
            ...
            className="flex-1 px-4 py-3 bg-white border border-slate-200 rounded-xl text-slate-900 placeholder-slate-400 focus:border-indigo-500 focus:ring-4 focus:ring-indigo-100 focus:outline-none"
          />
