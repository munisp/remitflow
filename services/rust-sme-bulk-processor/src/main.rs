...
async fn init_db() -> PgPool {
    ...
    sqlx::query(
        "CREATE TABLE IF NOT EXISTS sme_bulk_processor_state (
            id TEXT PRIMARY KEY,
    ...
    sqlx::query(
        "CREATE INDEX IF NOT EXISTS idx_sme_bulk_processor_updated ON sme_bulk_processor_state(updated_at)"
    )
    .execute(&pool)
    .await
    ...
    sqlx::query(
        "CREATE TABLE IF NOT EXISTS sme_bulk_processor_events (
    ...
    tracing::info!("PostgreSQL connected for rust-sme-bulk-processor");
    pool
}

async fn db_upsert(pool: &PgPool, id: &str, data: &serde_json::Value) -> Result<(), sqlx::Error> {
    sqlx::query(
        "INSERT INTO sme_bulk_processor_state (id, data, updated_at) VALUES ($1, $2, NOW())
...
async fn db_get(pool: &PgPool, id: &str) -> Result<Option<serde_json::Value>, sqlx::Error> {
    let row: Option<(serde_json::Value,)> = sqlx::query_as(
        "SELECT data FROM sme_bulk_processor_state WHERE id = $1"
    )
    .bind(id)
    .fetch_optional(pool)
    .await?;
...
async fn db_list(pool: &PgPool, limit: i64) -> Result<Vec<serde_json::Value>, sqlx::Error> {
    let rows: Vec<(serde_json::Value,)> = sqlx::query_as(
        "SELECT data FROM sme_bulk_processor_state ORDER BY updated_at DESC LIMIT $1"
    )
    .bind(limit)
    .fetch_all(pool)
    .await?;
...
async fn db_log_event(pool: &PgPool, event_type: &str, payload: &serde_json::Value) -> Result<(), sqlx::Error> {
    sqlx::query(
        "INSERT INTO sme_bulk_processor_events (event_type, payload) VALUES ($1, $2)"
    )
    .bind(event_type)
    .bind(payload)
    .execute(pool)
...
async fn load_from_db(pool: &PgPool) {
    match sqlx::query_as::<_, (String, serde_json::Value)>(
        "SELECT id, data FROM sme_bulk_processor_state ORDER BY updated_at DESC LIMIT 1000"
    )
    .fetch_all(pool)
    .await {
        Ok(rows) => {
            tracing::info!("loaded {} persisted records from sme_bulk_processor_state", rows.len());
        }
        Err(e) => {
...
async fn main() -> std::io::Result<()> {
    ...
    let (addr, server) = warp::serve(routes).bind_with_graceful_shutdown(
        ...
        async {
            ...
            eprintln!("[rust-sme-bulk-processor] Graceful shutdown initiated");
        eprintln!("{{\"event\":\"pod.shutdown.initiated\",\"service\":\"rust-sme-bulk-processor\",\"timestamp\":\"{}\"}}",
            chrono::Utc::now().to_rfc3339());;
        },
    );
    eprintln!("[rust-sme-bulk-processor] Listening on {}", addr);
    let startup_ms = _PROCESS_START.get_or_init(Instant::now).elapsed().as_millis();
    eprintln!("{{\"event\":\"pod.startup.complete\",\"service\":\"rust-sme-bulk-processor\",\"startup_ms\":{},\"timestamp\":\"{}\"}}",
