...
async def _prometheus_metrics():
    ...
    return Response(
        content=(
            ...
            f'pod_uptime_seconds{{service="python-ab-analysis"}} {uptime:.1f}\n'
            f"# HELP pod_ready Whether pod is ready\n"
            f"# TYPE pod_ready gauge\n"
            f'pod_ready{{service="python-ab-analysis"}} 1\n'
        ),
...
def _handle_shutdown(signum, frame):
    ...
    _shutdown_flag = True
    logging.getLogger("python-ab-analysis").info(f"Received signal {signum}, initiating graceful shutdown...")
...
async def _on_shutdown():
    logging.getLogger("python-ab-analysis").info("FastAPI shutdown event — cleaning up resources")
