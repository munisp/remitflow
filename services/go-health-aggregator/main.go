...
func loadFromDB() {
	...
	rows, err := db.Query("SELECT id, data FROM health_aggregator_state ORDER BY updated_at DESC LIMIT 1000")
	if err != nil {
	...
	slog.Info("loaded persisted state from database", "records", count, "table", "health_aggregator_state")
}
