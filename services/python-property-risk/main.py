...
def _require_env(name: str) -> str:
    ...
    if not value:
        raise RuntimeError(
            f"[python-property-risk] {name} is not set. Refusing to fall back to "
...
async def _prometheus_metrics():
    ...
    return Response(
        content=(
            ...
            f'pod_uptime_seconds{{service="python-property-risk"}} {uptime:.1f}\n'
            f"# HELP pod_ready Whether pod is ready\n"
            f"# TYPE pod_ready gauge\n"
            f'pod_ready{{service="python-property-risk"}} 1\n'
        ),
