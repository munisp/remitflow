...
class RiaError(Exception):
    """Provider error for the ria rail."""
    def __init__(self, code: str, message: str, details: Optional[Dict] = None):
        self.code = code
        self.message = message
        self.details = details or {}
        super().__init__(f"ria Error {code}: {message}")


class RiaClient:
    def __init__(self, api_key: str, base_url: str = "https://api.ria.com"):
    ...
    async def _request(self, method: str, path: str, body: Optional[Dict] = None,
        ...
        try:
            ...
            logger.error(f"ria HTTP {e.response.status_code} on {path}: {body_text}")
            raise RiaError(
                code=str(e.response.status_code),
                message=f"ria provider rejected the request",
                details={"path": path, "body": body_text}
            )
        except httpx.TimeoutException:
            logger.error(f"ria timeout on {path}")
            raise RiaError(code="TIMEOUT", message=f"Request to ria timed out", details={"path": path})
        except httpx.HTTPError as e:
            logger.error(f"ria network error on {path}: {e}")
            raise RiaError(code="NETWORK_ERROR", message=str(e), details={"path": path})

        try:
            data = response.json()
        except ValueError:
            raise RiaError(code="INVALID_RESPONSE", message="Non-JSON response from provider")
