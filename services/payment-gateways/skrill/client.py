...
class SkrillError(Exception):
    """Provider error for the skrill rail."""
    def __init__(self, code: str, message: str, details: Optional[Dict] = None):
        self.code = code
        self.message = message
        self.details = details or {}
        super().__init__(f"skrill Error {code}: {message}")


class SkrillClient:
    def __init__(self, api_key: str, base_url: str = "https://api.skrill.com"):
    ...
    async def _request(self, method: str, path: str, body: Optional[Dict] = None,
        ...
        try:
            ...
            logger.error(f"skrill HTTP {e.response.status_code} on {path}: {body_text}")
            raise SkrillError(
                code=str(e.response.status_code),
                message=f"skrill provider rejected the request",
                details={"path": path, "body": body_text}
            )
        except httpx.TimeoutException:
            logger.error(f"skrill timeout on {path}")
            raise SkrillError(code="TIMEOUT", message=f"Request to skrill timed out", details={"path": path})
        except httpx.HTTPError as e:
            logger.error(f"skrill network error on {path}: {e}")
            raise SkrillError(code="NETWORK_ERROR", message=str(e), details={"path": path})

        try:
            data = response.json()
        except ValueError:
            raise SkrillError(code="INVALID_RESPONSE", message="Non-JSON response from provider")
