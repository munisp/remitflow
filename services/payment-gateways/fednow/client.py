...
class FednowError(Exception):
    """Provider error for the fednow rail."""
    def __init__(self, code: str, message: str, details: Optional[Dict] = None):
        self.code = code
        self.message = message
        self.details = details or {}
        super().__init__(f"fednow Error {code}: {message}")


class FednowClient:
    def __init__(self, api_key: str, base_url: str = "https://api.fednow.com"):
    ...
    async def _request(self, method: str, path: str, body: Optional[Dict] = None,
        ...
        try:
            ...
            logger.error(f"fednow HTTP {e.response.status_code} on {path}: {body_text}")
            raise FednowError(
                code=str(e.response.status_code),
                message=f"fednow provider rejected the request",
                details={"path": path, "body": body_text}
            )
        except httpx.TimeoutException:
            logger.error(f"fednow timeout on {path}")
            raise FednowError(code="TIMEOUT", message=f"Request to fednow timed out", details={"path": path})
        except httpx.HTTPError as e:
            logger.error(f"fednow network error on {path}: {e}")
            raise FednowError(code="NETWORK_ERROR", message=str(e), details={"path": path})

        try:
            data = response.json()
        except ValueError:
            raise FednowError(code="INVALID_RESPONSE", message="Non-JSON response from provider")
