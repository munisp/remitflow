"""
TRANSFERGO Payment Gateway Client
"""
...
class TransfergoError(Exception):
    """Provider error for the transfergo rail."""
    def __init__(self, code: str, message: str, details: Optional[Dict] = None):
        self.code = code
        self.message = message
        self.details = details or {}
        super().__init__(f"transfergo Error {code}: {message}")


class TransfergoClient:
    def __init__(self, api_key: str, base_url: str = "https://api.transfergo.com"):
    ...
    async def _request(self, method: str, path: str, body: Optional[Dict] = None,
        ...
        try:
            ...
            logger.error(f"transfergo HTTP {e.response.status_code} on {path}: {body_text}")
            raise TransfergoError(
                code=str(e.response.status_code),
                message=f"transfergo provider rejected the request",
                details={"path": path, "body": body_text}
            )
        except httpx.TimeoutException:
            logger.error(f"transfergo timeout on {path}")
            raise TransfergoError(code="TIMEOUT", message=f"Request to transfergo timed out", details={"path": path})
        except httpx.HTTPError as e:
            logger.error(f"transfergo network error on {path}: {e}")
            raise TransfergoError(code="NETWORK_ERROR", message=str(e), details={"path": path})

        try:
            data = response.json()
        except ValueError:
            raise TransfergoError(code="INVALID_RESPONSE", message="Non-JSON response from provider")
