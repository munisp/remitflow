"""
AIRTEL-MONEY Payment Gateway Client
"""
...
class AirtelMoneyError(Exception):
    """Provider error for the airtel-money rail."""
    def __init__(self, code: str, message: str, details: Optional[Dict] = None):
        self.code = code
        self.message = message
        self.details = details or {}
        super().__init__(f"airtel-money Error {code}: {message}")


class AirtelMoneyClient:
    def __init__(self, api_key: str, base_url: str = "https://api.airtel-money.com"):
    ...
    async def _request(self, method: str, path: str, body: Optional[Dict] = None,
        ...
        try:
            ...
            logger.error(f"airtel-money HTTP {e.response.status_code} on {path}: {body_text}")
            raise AirtelMoneyError(
                code=str(e.response.status_code),
                message=f"airtel-money provider rejected the request",
                details={"path": path, "body": body_text}
            )
        except httpx.TimeoutException:
            logger.error(f"airtel-money timeout on {path}")
            raise AirtelMoneyError(code="TIMEOUT", message=f"Request to airtel-money timed out", details={"path": path})
        except httpx.HTTPError as e:
            logger.error(f"airtel-money network error on {path}: {e}")
            raise AirtelMoneyError(code="NETWORK_ERROR", message=str(e), details={"path": path})

        try:
            data = response.json()
        except ValueError:
            raise AirtelMoneyError(code="INVALID_RESPONSE", message="Non-JSON response from provider")
