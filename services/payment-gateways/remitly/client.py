"""
REMITLY Payment Gateway Client
"""
...
class RemitlyError(Exception):
    """Provider error for the remitly rail."""
    def __init__(self, code: str, message: str, details: Optional[Dict] = None):
        self.code = code
        self.message = message
        self.details = details or {}
        super().__init__(f"remitly Error {code}: {message}")
...
class RemitlyClient:
    ...
    async def _request(self, method: str, path: str, body: Optional[Dict] = None,
        ...
        try:
            ...
            logger.error(f"remitly HTTP {e.response.status_code} on {path}: {body_text}")
            raise RemitlyError(
                code=str(e.response.status_code),
                message=f"remitly provider rejected the request",
                details={"path": path, "body": body_text}
            )
        except httpx.TimeoutException:
            logger.error(f"remitly timeout on {path}")
            raise RemitlyError(code="TIMEOUT", message=f"Request to remitly timed out", details={"path": path})
        except httpx.HTTPError as e:
            logger.error(f"remitly network error on {path}: {e}")
            raise RemitlyError(code="NETWORK_ERROR", message=str(e), details={"path": path})

        try:
            data = response.json()
        except ValueError:
            raise RemitlyError(code="INVALID_RESPONSE", message="Non-JSON response from provider")
