...
class CipsError(Exception):
    """Provider error for the cips rail."""
    def __init__(self, code: str, message: str, details: Optional[Dict] = None):
        self.code = code
        self.message = message
        self.details = details or {}
        super().__init__(f"cips Error {code}: {message}")


class CipsClient:
    def __init__(self, api_key: str, base_url: str = "https://api.cips.com"):
    ...
    async def _request(self, method: str, path: str, body: Optional[Dict] = None,
        ...
        try:
            ...
            logger.error(f"cips HTTP {e.response.status_code} on {path}: {body_text}")
            raise CipsError(
                code=str(e.response.status_code),
                message=f"cips provider rejected the request",
                details={"path": path, "body": body_text}
            )
        except httpx.TimeoutException:
            logger.error(f"cips timeout on {path}")
            raise CipsError(code="TIMEOUT", message=f"Request to cips timed out", details={"path": path})
        except httpx.HTTPError as e:
            logger.error(f"cips network error on {path}: {e}")
            raise CipsError(code="NETWORK_ERROR", message=str(e), details={"path": path})

        try:
            data = response.json()
        except ValueError:
            raise CipsError(code="INVALID_RESPONSE", message="Non-JSON response from provider")
