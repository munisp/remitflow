...
def _require_env(name: str) -> str:
    ...
    if not value:
        raise RuntimeError(
            f"[python-synthetic-monitor] {name} is not set. Refusing to fall back to "
    ...
    if _db_pool is None:
        ...
        with _db_pool.cursor() as cur:
            cur.execute("""
                ...
                CREATE INDEX IF NOT EXISTS idx_synthetic_monitor_updated
                    ON synthetic_monitor_state(updated_at);
                ...
                CREATE INDEX IF NOT EXISTS idx_synthetic_monitor_events_type
                    ON synthetic_monitor_events(event_type, created_at);
            """)
    return _db_pool
...
def db_upsert(record_id: str, data: dict):
    ...
    with conn.cursor() as cur:
        cur.execute(
            """INSERT INTO synthetic_monitor_state (id, data, updated_at)
               VALUES (%s, %s, NOW())
...
def db_get(record_id: str) -> dict | None:
    ...
    with conn.cursor(cursor_factory=psycopg2.extras.RealDictCursor) as cur:
        cur.execute("SELECT data FROM synthetic_monitor_state WHERE id = %s", (record_id,))
        row = cur.fetchone()
...
def db_list(limit: int = 100) -> list[dict]:
    ...
    with conn.cursor(cursor_factory=psycopg2.extras.RealDictCursor) as cur:
        cur.execute(
            "SELECT data FROM synthetic_monitor_state ORDER BY updated_at DESC LIMIT %s",
            (limit,)
        )
...
def db_log_event(event_type: str, payload: dict):
    ...
    with conn.cursor() as cur:
        cur.execute(
            "INSERT INTO synthetic_monitor_events (event_type, payload) VALUES (%s, %s)",
