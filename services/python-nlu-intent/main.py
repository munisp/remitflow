...
def _get_db():
    ...
    if _db_pool is None:
        ...
        with _db_pool.cursor() as cur:
            cur.execute("""
                CREATE TABLE IF NOT EXISTS nlu_intent_state (
                    id TEXT PRIMARY KEY,
                    data JSONB NOT NULL DEFAULT '{}',
                    created_at TIMESTAMPTZ NOT NULL DEFAULT NOW(),
                    updated_at TIMESTAMPTZ NOT NULL DEFAULT NOW()
                );
                CREATE INDEX IF NOT EXISTS idx_nlu_intent_updated
                    ON nlu_intent_state(updated_at);
                CREATE TABLE IF NOT EXISTS nlu_intent_events (
                    id BIGSERIAL PRIMARY KEY,
                    event_type TEXT NOT NULL,
                    payload JSONB NOT NULL DEFAULT '{}',
                    created_at TIMESTAMPTZ NOT NULL DEFAULT NOW()
                );
                CREATE INDEX IF NOT EXISTS idx_nlu_intent_events_type
                    ON nlu_intent_events(event_type, created_at);
            """)
    return _db_pool
...
def db_upsert(record_id: str, data: dict):
    ...
    with conn.cursor() as cur:
        cur.execute(
            """INSERT INTO nlu_intent_state (id, data, updated_at)
               VALUES (%s, %s, NOW())
...
def db_get(record_id: str) -> dict | None:
    ...
    with conn.cursor(cursor_factory=psycopg2.extras.RealDictCursor) as cur:
        cur.execute("SELECT data FROM nlu_intent_state WHERE id = %s", (record_id,))
        row = cur.fetchone()
...
def db_list(limit: int = 100) -> list[dict]:
    ...
    with conn.cursor(cursor_factory=psycopg2.extras.RealDictCursor) as cur:
        cur.execute(
            "SELECT data FROM nlu_intent_state ORDER BY updated_at DESC LIMIT %s",
            (limit,)
        )
...
def db_log_event(event_type: str, payload: dict):
    ...
    with conn.cursor() as cur:
        cur.execute(
            "INSERT INTO nlu_intent_events (event_type, payload) VALUES (%s, %s)",
...
# _metadata — persisted to PostgreSQL table "nlu_intent_metadata" (see _db__metadata_* helpers)

class _DbMetadata:
    """PostgreSQL-backed store replacing in-memory dict '_metadata'."""
    TABLE = "nlu_intent_metadata"
...
async def _prometheus_metrics():
    ...
    return Response(
        content=(
            ...
            f'pod_uptime_seconds{{service="python-nlu-intent"}} {uptime:.1f}\n'
            f"# HELP pod_ready Whether pod is ready\n"
            f"# TYPE pod_ready gauge\n"
            f'pod_ready{{service="python-nlu-intent"}} 1\n'
        ),
...
def init_pg_tables():
    ...
    try:
        _db_exec("""
            CREATE TABLE IF NOT EXISTS nlu_intent_metadata (
