...
# _norm_params — persisted to PostgreSQL table "fx_norm_params" (see _db__norm_params_* helpers)

class _DbNormParams:
    """PostgreSQL-backed store replacing in-memory dict '_norm_params'."""
    TABLE = "fx_norm_params"
...
_norm_params = _DbNormParams()
# _metadata — persisted to PostgreSQL table "fx_forecast_metadata" (see _db__metadata_* helpers)

class _DbMetadata:
    """PostgreSQL-backed store replacing in-memory dict '_metadata'."""
    TABLE = "fx_forecast_metadata"
...
async def _prometheus_metrics():
    ...
    return Response(
        content=(
            ...
            f'pod_uptime_seconds{{service="python-fx-forecasting"}} {uptime:.1f}\n'
            f"# HELP pod_ready Whether pod is ready\n"
            f"# TYPE pod_ready gauge\n"
            f'pod_ready{{service="python-fx-forecasting"}} 1\n'
        ),
...
def model_info():
    ...
    db_items = _db_list("fx_forecasting", 200)
    if db_items:
...
async def forecast(req: ForecastRequest):
    ...
    try:
        _result_data = ForecastResponse if isinstance(ForecastResponse, dict) else {"result": str(ForecastResponse)}
        _db_upsert("fx_forecasting", f"forecast:{int(_time.time()*1000)}", _result_data)
        _db_log_event("fx_forecasting", "forecast", _result_data)
    except Exception:
        pass
...
async def trigger_train():
    ...
    try:
        ...
        _db_upsert("fx_forecasting", f"train:{int(_time.time()*1000)}", _result_data)
        _db_log_event("fx_forecasting", "train", _result_data)
    except Exception:
        pass
...
def init_pg_tables():
    ...
    try:
        _db_exec("""
            CREATE TABLE IF NOT EXISTS fx_norm_params (
        ...
        """)
        _db_exec("""
            CREATE TABLE IF NOT EXISTS fx_forecast_metadata (
            key TEXT PRIMARY KEY,
            data JSONB NOT NULL DEFAULT '{}'::jsonb,
            updated_at TIMESTAMPTZ DEFAULT NOW()
            )
