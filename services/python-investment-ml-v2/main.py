...
_alloc_bundle = None
# _metadata — persisted to PostgreSQL table "investment_ml_v2_metadata" (see _db__metadata_* helpers)

class _DbMetadata:
    """PostgreSQL-backed store replacing in-memory dict '_metadata'."""
    TABLE = "investment_ml_v2_metadata"
...
async def _prometheus_metrics():
    ...
    return Response(
        content=(
            ...
            f'pod_uptime_seconds{{service="python-investment-ml-v2"}} {uptime:.1f}\n'
            f"# HELP pod_ready Whether pod is ready\n"
            f"# TYPE pod_ready gauge\n"
            f'pod_ready{{service="python-investment-ml-v2"}} 1\n'
        ),
...
def _handle_shutdown(signum, frame):
    ...
    _shutdown_flag = True
    logging.getLogger("python-investment-ml-v2").info(f"Received signal {signum}, initiating graceful shutdown...")
...
def _emit_lifecycle_event(event_type: str, **kwargs):
    ...
    payload = {
        "event": event_type,
        "service": "python-investment-ml-v2",
...
async def _on_shutdown():
    logging.getLogger("python-investment-ml-v2").info("FastAPI shutdown event — cleaning up resources")
...
def init_pg_tables():
    ...
    try:
        _db_exec("""
            CREATE TABLE IF NOT EXISTS investment_ml_v2_metadata (
