...
fn init_db() {
    ...
    match Client::connect(&dsn, NoTls) {
        Ok(mut client) => {
            ...
            eprintln!("[PQ-CRYPTO] PostgreSQL connected, table pq_crypto_state ready");
        }
        Err(e) => {
            eprintln!("[PQ-CRYPTO] WARN: PostgreSQL unavailable: {}", e);
        }
    }
}
...
fn db_upsert(id: &str, data: &serde_json::Value) {
    if let Some(client) = DB_CLIENT.get() {
        if let Ok(mut c) = client.lock() {
            let _ = c.execute(
                "INSERT INTO pq_crypto_state (id, data, updated_at) VALUES ($1, $2, NOW()) ON CONFLICT (id) DO UPDATE SET data = EXCLUDED.data, updated_at = NOW()",
                &[&id, &data],
            );
        }
    }
...
fn load_from_db() {
    if let Some(client) = DB_CLIENT.get() {
        if let Ok(mut c) = client.lock() {
            match c.query("SELECT id, data FROM pq_crypto_state LIMIT 1000", &[]) {
                Ok(rows) => eprintln!("[PQ-CRYPTO] loaded {} persisted records from pq_crypto_state", rows.len()),
                Err(e) => eprintln!("[PQ-CRYPTO] WARN: failed to load from DB: {}", e),
            }
        }
    }
}
...
fn handle_generate_key(body: &str, state: &AppState) -> String {
    ...
    std::thread::spawn(move || {
        db_upsert(&format!("key:{}", kid), &serde_json::json!({"key_type": kt, "status": "active"}));
    });
...
fn handle_tokenize(body: &str, state: &AppState) -> String {
    ...
    std::thread::spawn(move || {
        db_upsert(&format!("token:{}", tok), &serde_json::json!({"field_type": ft}));
    });
