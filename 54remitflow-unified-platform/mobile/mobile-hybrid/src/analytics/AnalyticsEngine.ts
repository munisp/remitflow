...
class AnalyticsEngine {
  ...
  async trackFeatureUsage(featureName: string): Promise<void> {
    const stored = await AsyncStorage.getItem(`feature_${featureName}`);
    ...
    await AsyncStorage.setItem(`feature_${featureName}`, JSON.stringify(adoption));
  ...
  async trackRetention(): Promise<void> {
    const installDateStr = await AsyncStorage.getItem('install_date');
    const installDate = installDateStr ? parseInt(installDateStr) : Date.now();

    if (!installDateStr) {
      await AsyncStorage.setItem('install_date', installDate.toString());
    }
  ...
  async trackScreenView(screenName: string, params?: Record<string, any>): Promise<void> {
    ...
    await AsyncStorage.setItem(`session_${this.sessionId}_screens`, (count + 1).toString());
  ...
  async trackButtonClick(buttonName: string, screenName: string, metadata?: Record<string, any>): Promise<void> {
    ...
    await AsyncStorage.setItem(`session_${this.sessionId}_clicks`, (count + 1).toString());
  ...
  async trackError(errorType: string, errorMessage: string, stackTrace?: string): Promise<void> {
    ...
    await AsyncStorage.setItem(`session_${this.sessionId}_errors`, (count + 1).toString());
  ...
  private async getDeviceInfo(): Promise<DeviceInfo> {
    return {
      ...
      deviceModel: Capacitor.getPlatform() === 'ios' ? 'iPhone' : 'Android',
  ...
  private async getScreenViewCount(): Promise<number> {
    const count = await AsyncStorage.getItem(`session_${this.sessionId}_screens`);
    return count ? parseInt(count) : 0;
  }

  private async getClickCount(): Promise<number> {
    const count = await AsyncStorage.getItem(`session_${this.sessionId}_clicks`);
    return count ? parseInt(count) : 0;
  }

  private async getErrorCount(): Promise<number> {
    const count = await AsyncStorage.getItem(`session_${this.sessionId}_errors`);
    return count ? parseInt(count) : 0;
  }
