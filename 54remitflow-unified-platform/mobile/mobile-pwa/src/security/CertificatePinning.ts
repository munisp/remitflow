...
import RNSSLPinning from 'ssl-pinning';
...
class CertificatePinning {
  ...
  private initializePinning(): void {
    ...
    const apiHost = process.env.VITE_API_HOST || 'api.agentbanking.com';
    const authHost = process.env.VITE_AUTH_HOST || 'auth.agentbanking.com';
    const paymentHost = process.env.VITE_PAYMENT_HOST || 'payment.agentbanking.com';

    this.addPinnedDomain({
      hostname: apiHost,
      publicKeyHashes: [
        process.env.VITE_PIN_API_PRIMARY || 'sha256/CONFIGURE_PRIMARY_CERT_HASH_BEFORE_PRODUCTION',
        process.env.VITE_PIN_API_BACKUP || 'sha256/CONFIGURE_BACKUP_CERT_HASH_BEFORE_PRODUCTION',
      ],
    ...
    this.addPinnedDomain({
      hostname: authHost,
      publicKeyHashes: [
        process.env.VITE_PIN_AUTH_PRIMARY || 'sha256/CONFIGURE_AUTH_CERT_HASH_BEFORE_PRODUCTION',
        process.env.VITE_PIN_AUTH_BACKUP || 'sha256/CONFIGURE_AUTH_BACKUP_HASH_BEFORE_PRODUCTION',
      ],
    ...
    this.addPinnedDomain({
      ...
      publicKeyHashes: [
        process.env.VITE_PIN_PAY_PRIMARY || 'sha256/CONFIGURE_PAYMENT_CERT_HASH_BEFORE_PRODUCTION',
        process.env.VITE_PIN_PAY_BACKUP || 'sha256/CONFIGURE_PAYMENT_BACKUP_HASH_BEFORE_PRODUCTION',
      ],
      includeSubdomains: false,
    });
  }
  ...
  private loadPinsFromEnv(): PinningConfig[] {
    const pinsJson = process.env.VITE_CERT_PINS;
    if (!pinsJson) return [];
    try {
      return JSON.parse(pinsJson) as PinningConfig[];
    } catch {
      console.error('[SECURITY] Failed to parse VITE_CERT_PINS env var');
      return [];
    }
  }
