...
export const SecurityDashboardScreen = ({ navigation }: any) => {
  ...
  const [tab, setTab] = useState<'denials'|'anomalies'>('denials');

  useEffect(() => {
    AnalyticsService.trackScreenView('SecurityDashboard');
    loadData();
  }, []);
  ...
  const loadData = async () => {
    setLoading(true);
    try {
      const [d, a] = await Promise.all([apiClient.get('/api/trpc/securityAudit.pbacDenyEvents'), apiClient.get('/api/trpc/securityAudit.anomalyAlerts')]);
      setDenials(d?.result?.data ?? []); setAnomalies(a?.result?.data ?? []);
    } catch (e) {
      AnalyticsService.trackError('securitydashboard_load_failed', e);
    } finally {
      setLoading(false);
    }
  };
  ...
  const renderContent = () => (
    <ScrollView>
      <View style={{ flexDirection: 'row', margin: 16, borderRadius: 8, overflow: 'hidden', borderWidth: 1, borderColor: '#6366f1' }}>
        {(['denials', 'anomalies'] as const).map(t => (
      ...
      {tab === 'denials' ? (
        denials.filter(d => d.action?.toLowerCase().includes(search.toLowerCase()) || !search).map((d, i) => (
          <View key={i} style={[styles.card, { borderLeftWidth: 3, borderLeftColor: '#ef4444' }]}>
            <Text style={styles.cardTitle}>{d.action ?? 'PBAC Denial'}</Text>
            <Text style={styles.cardSubtitle}>User: {d.userId ?? 'N/A'} · Resource: {d.resource ?? 'N/A'}</Text>
            <Text style={styles.label}>{d.timestamp ? new Date(d.timestamp).toLocaleString() : 'N/A'}</Text>
          </View>
        ))
      ) : (
        anomalies.filter(a => a.type?.toLowerCase().includes(search.toLowerCase()) || !search).map((a, i) => (
          <View key={i} style={[styles.card, { borderLeftWidth: 3, borderLeftColor: '#f59e0b' }]}>
            <Text style={styles.cardTitle}>{a.type ?? 'Anomaly'}</Text>
            <Text style={styles.cardSubtitle}>Score: {a.score ?? 0} · {a.description ?? ''}</Text>
            <Text style={styles.label}>{a.timestamp ? new Date(a.timestamp).toLocaleString() : 'N/A'}</Text>
          </View>
        ))
      )}
    </ScrollView>
