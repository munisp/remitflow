          ...
          : Column(children: [
              ...
              Expanded(child: RefreshIndicator(
                ...
                child: ListView(children: [
                  ..._payments.where((p) => (p['description'] ?? '').toString().toLowerCase().contains(_search.toLowerCase())).map((p) => _card(
                    title: p['description'] ?? 'Recurring Payment',
                    amount: '\$\${(p['amount'] ?? 0).toStringAsFixed(2)}',
                    subtitle: '\${p['frequency']} · Next: \${p['nextRunDate'] ?? 'N/A'}',
                    actions: [
                      _actionBtn(p['status'] == 'paused' ? 'Resume' : 'Pause', () => ScaffoldMessenger.of(context).showSnackBar(SnackBar(content: Text('\${p['status'] == 'paused' ? 'Resumed' : 'Paused'}'))), color: p['status'] == 'paused' ? const Color(0xFF22c55e) : const Color(0xFFf59e0b)),
                      const SizedBox(width: 8),
                      _actionBtn('Cancel', () => ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text('Cancelled'))), color: const Color(0xFFef4444)),
                    ],
                  )).toList(),
                  if (_payments.isEmpty) const Center(child: Padding(padding: EdgeInsets.all(40), child: Text('No recurring payments.', style: TextStyle(color: Colors.grey)))),
                ]),
              )),
            ]),
    );
