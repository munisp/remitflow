...
class _DirectDebitScreenState extends State<DirectDebitScreen> {
  ...
  Future<void> _loadData() async {
    ...
    try {
      ...
      if (mounted) setState(() => _mandates = []);
    } catch (e) {
      debugPrint('Error loading Direct Debit: $e');
    } finally {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      appBar: AppBar(
        ...
        title: const Text('Direct Debit', style: TextStyle(color: Colors.white)),
          ...
          : Column(children: [
              ...
              Expanded(child: RefreshIndicator(
                ...
                child: ListView(children: [
                  ..._mandates.where((m) => (m['creditorName'] ?? '').toString().toLowerCase().contains(_search.toLowerCase())).map((m) => _card(
                    title: m['creditorName'] ?? 'Direct Debit',
                    amount: '\$\${(m['amount'] ?? 0).toStringAsFixed(2)}',
                    subtitle: '\${m['frequency'] ?? 'monthly'} · Next: \${m['nextPaymentDate'] ?? 'N/A'}',
                    label: 'Ref: \${m['reference'] ?? 'N/A'}',
                    actions: m['status'] == 'active' ? [
                      _actionBtn('Cancel Mandate', () => ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text('Mandate cancelled'))), color: const Color(0xFFef4444)),
                    ] : null,
                  )).toList(),
                  if (_mandates.isEmpty) const Center(child: Padding(padding: EdgeInsets.all(40), child: Text('No direct debit mandates.', style: TextStyle(color: Colors.grey)))),
                ]),
              )),
            ]),
    );
