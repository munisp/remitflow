...
class BatchPaymentsScreen extends StatefulWidget {
  const BatchPaymentsScreen({Key? key}) : super(key: key);
  @override
  State<BatchPaymentsScreen> createState() => _BatchPaymentsScreenState();
}

class _BatchPaymentsScreenState extends State<BatchPaymentsScreen> {
  ...
  Future<void> _loadData() async {
    ...
    try {
      ...
      debugPrint('Error loading Batch Payments: $e');
    } finally {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      appBar: AppBar(
        ...
        title: const Text('Batch Payments', style: TextStyle(color: Colors.white)),
          ...
          : Column(children: [
              ...
              Expanded(child: RefreshIndicator(
                ...
                child: ListView(children: [
                  ..._batches.where((b) => (b['name'] ?? '').toString().toLowerCase().contains(_search.toLowerCase())).map((b) => _card(
                    title: b['name'] ?? 'Batch #\${b['id']}',
                    subtitle: '\${b['totalRecipients'] ?? 0} recipients · \$\${(b['totalAmount'] ?? 0).toStringAsFixed(2)}',
                    label: '✓ \${b['successCount'] ?? 0} success  ✗ \${b['failureCount'] ?? 0} failed',
                  )).toList(),
                  if (_batches.isEmpty) const Center(child: Padding(padding: EdgeInsets.all(40), child: Text('No batch payments.', style: TextStyle(color: Colors.grey)))),
                ]),
              )),
            ]),
    );
