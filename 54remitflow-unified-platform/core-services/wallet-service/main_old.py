...
class WalletServiceResponse(BaseModel):
    id: str
    status: str
    message: str
    data: Optional[dict] = None
...
@app.post("/api/v1/wallet-service/create", response_model=WalletServiceResponse)
async def create(request: WalletServiceRequest):
    # Implementation here
    return {
...
@app.get("/api/v1/wallet-service/{item_id}")
