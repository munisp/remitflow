...
```bash
export SNAPCHAT_API_KEY="your_api_key"
export SNAPCHAT_API_SECRET="your_api_secret"
export SNAPCHAT_WEBHOOK_SECRET="your_webhook_secret"
export PORT=8096
```
...
Visit `http://localhost:8096/docs` for interactive API documentation.
...
```bash
curl -X POST http://localhost:8096/api/v1/send \
  -H "Content-Type: application/json" \
  -d '{
    "recipient": "+1234567890",
    "message_type": "text",
    "content": "Hello from Snapchat!"
  }'
```
...
```bash
curl -X POST http://localhost:8096/api/v1/order \
  -H "Content-Type: application/json" \
  -d '{
    "customer_id": "CUST123",
