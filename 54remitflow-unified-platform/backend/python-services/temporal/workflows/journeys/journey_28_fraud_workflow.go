...
// FraudDetectionWorkflow orchestrates the Fraud Detection journey
func FraudDetectionWorkflow(ctx workflow.Context, input FraudDetectionWorkflowInput) (*FraudDetectionWorkflowResult, error) {
	logger := workflow.GetLogger(ctx)
	logger.Info("FraudDetectionWorkflow started", "user_id", input.UserID)
	...
	result := &FraudDetectionWorkflowResult{
		Success: true,
	...
	result.Message = "Fraud Detection completed successfully"
	logger.Info("FraudDetectionWorkflow completed", "user_id", input.UserID)
