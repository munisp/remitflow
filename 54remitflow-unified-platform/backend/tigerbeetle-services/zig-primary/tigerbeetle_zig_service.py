...
class TigerBeetleZigService:
    ...
    async def download_tigerbeetle(self):
        ...
        try:
            ...
            # FAIL CLOSED - Do not use mock implementation in production
            # This prevents data loss from in-memory storage
            raise RuntimeError(
                f"CRITICAL: TigerBeetle binary not available and download failed: {str(e)}. "
                "Cannot start service without TigerBeetle. "
                "Please ensure TigerBeetle is installed at /usr/local/bin/tigerbeetle "
                "or set TIGERBEETLE_BINARY_PATH environment variable."
            )
    
    async def init_tigerbeetle_client(self):
        """Initialize TigerBeetle client"""
        # Verify TigerBeetle process is running before initializing client
        if self.tigerbeetle_process is None or self.tigerbeetle_process.poll() is not None:
            raise RuntimeError(
                "CRITICAL: TigerBeetle process is not running. "
                "Cannot initialize client without running TigerBeetle server."
            )
        
        # Initialize in-memory cache for performance (backed by TigerBeetle)
        # These are caches only - TigerBeetle is the source of truth
        self.tigerbeetle_accounts = {}
        self.tigerbeetle_transfers = {}
        self._tigerbeetle_initialized = True
        logger.info("TigerBeetle client initialized (connected to TigerBeetle server)")
