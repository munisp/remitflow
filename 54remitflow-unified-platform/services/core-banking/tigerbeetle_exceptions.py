...
class InsufficientBalanceError(AccountError):
    ...
    def __init__(self, account_id: int, required: int, available: int):
        super().__init__(
...
class ConnectionError(TigerBeetleError):
    ...
    def __init__(self, replica_address: str, reason: Optional[str] = None):
        super().__init__(
...
class PartialBatchFailureError(BatchError):
    ...
    def __init__(self, total: int, successful: int, failed: int, errors: list):
        super().__init__(
