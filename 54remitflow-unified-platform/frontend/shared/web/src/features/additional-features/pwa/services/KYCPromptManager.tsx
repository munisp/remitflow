...
const fetchUserKYCData = async (): Promise<UserKYCData> => {
  const res = await fetch(`${API_BASE_URL}/kyc.getStatus`, {
    method: 'GET',
    credentials: 'include',
    headers: { 'Content-Type': 'application/json' },
  });
  if (!res.ok) throw new Error(`Failed to fetch KYC data: ${res.status}`);
  const json = await res.json();
  const data = json?.result?.data;
  if (!data) throw new Error('Invalid KYC response structure');
  return {
    status: data.status ?? 'REQUIRED',
    currentLimit: data.monthlyLimit ?? 5000,
    usedAmount: data.monthlyUsed ?? 0,
    limitCurrency: data.currency ?? 'USD',
    nextUpgradeLevel: data.nextTier ?? 'Level 2 (Increased Limits)',
    upgradeLink: '/settings/kyc',
  };
};
...
const logPromptImpression = async (type: 'UPGRADE' | 'WARNING'): Promise<void> => {
  try {
    await fetch(`${API_BASE_URL}/audit.logEvent`, {
      method: 'POST',
      credentials: 'include',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ json: { event: `KYC_PROMPT_${type}`, timestamp: Date.now() } }),
    });
  } catch {
