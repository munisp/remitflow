...
const CommissionRulesManager = () => {
  ...
  const handleUpdateRule = async () => {
    try {
      const response = await fetch(`/api/commission-rules/${editingRule.id}`, {
        method: 'PUT',
        headers: {
          'Content-Type': 'application/json',
        },
  ...
  const handleToggleActive = async (ruleId) => {
    try {
      ...
      const updatedStatus = { is_active: !ruleToUpdate.is_active };

      const response = await fetch(`/api/commission-rules/${ruleId}/status`, {
        method: 'PATCH',
        headers: {
          'Content-Type': 'application/json',
        },
