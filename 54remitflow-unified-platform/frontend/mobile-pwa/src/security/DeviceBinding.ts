...
class DeviceBinding {
  ...
  async generateFingerprint(): Promise<DeviceFingerprint> {
    ...
    const screenResolution = `${'web' === 'web' ? window.screen.width : 0}x${'web' === 'web' ? window.screen.height : 0}`;
  ...
  async getTrustedDevices(): Promise<DeviceFingerprint[]> {
    // Load from storage
    const stored = await localforage.getItem('trusted_devices_data');
    if (stored) {
  ...
  private async loadTrustedDevices(): Promise<void> {
    try {
      const stored = await localforage.getItem('trusted_devices');
      if (stored) {
  ...
  private async saveTrustedDevices(): Promise<void> {
    try {
      ...
      await localforage.setItem('trusted_devices', JSON.stringify(devices));
    } catch (error) {
  ...
  private async updateDeviceLastSeen(fingerprint: DeviceFingerprint): Promise<void> {
    ...
    await localforage.setItem('trusted_devices_data', JSON.stringify(updated));
  }
