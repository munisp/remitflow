                    ...
                    Task {
                        if let data = try? await selectedItem?.loadTransferable(type: Data.self),
                           let uiImage = UIImage(data: data) {
                            kycData.selfieImage = uiImage
                            livenessViewModel.checkLiveness(selfieImage: uiImage)
                        }
                    }
                }

                switch livenessViewModel.livenessState {
                case .loading:
                    ProgressView()
                case .success(let isLive, let confidence):
                    Text("Liveness: \(isLive ? "Passed" : "Failed") (Confidence: \(confidence))")
...
struct SelfieVerificationStep: View {
    @StateObject private var livenessViewModel = LivenessViewModel()
