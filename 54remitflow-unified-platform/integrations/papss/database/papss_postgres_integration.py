...
class PAPSSPostgresIntegration:
    ...
    def update_payment_status(self, payment_id: str, status: str, error_message: str = None):
    ...
    def store_fx_rate(self, source_currency: str, target_currency: str, rate: Decimal,
                      provider: str, valid_duration_minutes: int = 60):
        """Store FX rate"""
        conn = self._get_connection()
        try:
            cursor = conn.cursor()
    ...
    def _log_audit_trail(self, payment_id: str, action: str, details: Dict[str, Any]):
        """Log audit trail"""
        conn = self._get_connection()
        try:
            cursor = conn.cursor()
