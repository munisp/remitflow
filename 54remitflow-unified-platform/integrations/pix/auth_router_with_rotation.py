...
async def login(
    ...
    if not user:
        ...
        audit_logger.log_login_failed(form_data.username, request, "Invalid credentials")
        
        # Record in monitoring system
        monitoring = MonitoringService(db)
        monitoring.record_failed_login(
    ...
    audit_logger.log_login_success(
        user=user,
        request=request,
        token_family_id=refresh_token.split('.')[0],  # Use first part as family ID
        device_info=client_info["device_info"]
    )
    
    # Record in monitoring system
    monitoring = MonitoringService(db)
    monitoring.record_successful_login(
        username=user.username,
        ip_address=client_info["ip_address"] or "unknown"
    )
