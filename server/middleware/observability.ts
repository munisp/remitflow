...
async function _ensureWriteThroughTables(): Promise<void> {
  ...
  try {
    const { sql } = await import("drizzle-orm");
    await (db as any).execute(sql`
      CREATE TABLE IF NOT EXISTS observability_error_budgets (
...
const errorBudgets = new Map<string, { consumed: number; windowStart: number }>(); // Persisted to PostgreSQL table "observability_error_budgets"
...
export function trackErrorBudget(sloName: string, errorOccurred: boolean): void {
  ...
  _writeThrough("observability_error_budgets", key, existing).catch(() => {});
}
