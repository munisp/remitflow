...
export class TigerBeetleIntegration {
  ...
  async postPendingTransfer(params: {
    ...
    if (results && results.length > 0) {
      // TB exists(46) = idempotent replay of an identical post (deterministic
      // post id) — dedup success, not failure. already_posted(33) and every
      // exists_with_different_* remain hard errors.
      const errors = results.filter((r: { result: number }) => r.result !== 0 && r.result !== 46);
      if (errors.length > 0) {
  ...
  async voidPendingTransfer(params: {
    ...
    if (results && results.length > 0) {
      // TB exists(46) = idempotent replay of an identical void (deterministic
      // void id) — dedup success, not failure. already_voided(34) etc. remain
      // hard errors so callers can inspect the hold's true state.
      const errors = results.filter((r: { result: number }) => r.result !== 0 && r.result !== 46);
      if (errors.length > 0) {
