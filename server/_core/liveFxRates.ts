...
import { persistFeatureRecord } from "./featurePersistence";
...
async function _ensureWriteThroughTables(): Promise<void> {
  ...
  try {
    const { sql } = await import("drizzle-orm");
    await (db as any).execute(sql`
      CREATE TABLE IF NOT EXISTS live_fx_rate_cache (
...
const cache = new Map<string, CacheEntry<unknown>>(); // Persisted to PostgreSQL table "live_fx_rate_cache"
