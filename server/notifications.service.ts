...
export async function sendNotification(
  ...
  try {
    ...
    if (db) {
      // Map notification type to DB enum (DB uses 'transaction' instead of 'transfer')
      const dbType = payload.type === 'transfer' ? 'transaction'
        : payload.type === 'payment' ? 'transaction'
        : payload.type === 'referral' ? 'promotion'
        : payload.type;
      await db.execute(
        sql`INSERT INTO notifications ("userId", title, message, type, "isRead", "createdAt")
         VALUES (${payload.userId}, ${payload.title}, ${payload.message}, ${dbType}::notif_type, false, NOW())`
      );
      result.db = true;
    }
  } catch (error) {
