...
export const cbnComplianceRouter = router({
  ...
  approveBdcPartner: adminProcedure
    ...
    .mutation(async ({ ctx, input }) => {
      // W13 (F-17): TOTP step-up + guarded single-winner pending_review→approved.
      await requireTotpStepUp(ctx.user.id, input.totpCode, "BDC partner approval");
      const db = await getDb();
      const [partner] = await db
        .update(bdcPartners)
        .set({
        ...
        .where(and(eq(bdcPartners.id, input.id), eq(bdcPartners.status, "pending_review")))
        .returning();

      if (!partner) {
        const [existing] = await db.select({ status: bdcPartners.status }).from(bdcPartners)
          .where(eq(bdcPartners.id, input.id)).limit(1);
        if (!existing) throw new TRPCError({ code: "NOT_FOUND", message: `BDC partner ${input.id} not found` });
        throw new TRPCError({
          code: "CONFLICT",
          message: `BDC partner ${input.id} cannot be approved from status '${existing.status}' (only pending_review)`,
        });
      }

      await createAuditLog({
        userId: ctx.user.id,
        action: "BDC_PARTNER_APPROVED",
        targetType: "bdc_partners",
        targetId: partner.id,
        description: `BDC partner '${partner.name}' (licence ${partner.cbnLicenceNumber}) approved`,
        metadata: { partnerId: partner.id, cbnLicenceNumber: partner.cbnLicenceNumber },
      });

      // W13 (F-17): the fabricated Keycloak temp-password onboarding block was
      // DELETED — no Keycloak account or gateway credential is created by this
      // procedure, so nothing may claim otherwise. Notifications are an honest
      // approval notice; portal credentials are provisioned separately.
      await notifyOwner({
        ...
        content: [
          ...
          ``,
          `NOTE: portal/API credentials are NOT created by this approval —`,
          `they are provisioned separately by the platform team.`,
          ``,
      ...
      // Send approval notice to the BDC partner's compliance contact
      if (partner.contactEmail) {
        try {
          await sendEmail({
            to: partner.contactEmail,
            subject: `RemitFlow CBN BDC Network — Application Approved: ${partner.name}`,
            html: `<div style="font-family:sans-serif;max-width:560px;margin:0 auto;padding:32px">
    <h2 style="margin:0 0 8px">Your Application Has Been Approved</h2>
    <p><strong>${partner.name}</strong> has been approved as a licensed BDC partner on the RemitFlow CBN network.</p>
    <table style="width:100%;border-collapse:collapse;font-size:14px;margin-bottom:24px">
      <tr><td style="padding:10px 0;color:#64748b">CBN Licence</td><td style="padding:10px 0;font-weight:600">${partner.cbnLicenceNumber}</td></tr>
      <tr><td style="padding:10px 0;color:#64748b">ADB Name</td><td style="padding:10px 0">${partner.adbName}</td></tr>
      <tr><td style="padding:10px 0;color:#64748b">Daily FX Limit</td><td style="padding:10px 0;font-weight:600">$${(partner.maxDailyFxUsd ?? 100000).toLocaleString()} USD</td></tr>
      <tr><td style="padding:10px 0;color:#64748b">Approved At</td><td style="padding:10px 0">${new Date().toUTCString()}</td></tr>
    </table>
    <p style="font-size:13px;color:#475569">Portal and API credentials are provisioned separately — you will receive them from the RemitFlow platform team. No credentials are included in this email.</p>
    <p style="font-size:12px;color:#94a3b8">This email was auto-generated by the RemitFlow CBN Compliance System.</p>
</div>`,
            text: `${partner.name} has been approved as a BDC partner on the RemitFlow CBN network.\nCBN Licence: ${partner.cbnLicenceNumber}\nADB Name: ${partner.adbName}\nDaily FX Limit: $${(partner.maxDailyFxUsd ?? 100000).toLocaleString()} USD\n\nPortal and API credentials are provisioned separately by the platform team.`,
          });
        } catch (emailErr) {
          logger.warn({ data: emailErr }, '[BDC Onboarding] Approval email delivery failed:');
        }
      }
