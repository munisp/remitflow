...
export const smartRoutingRouter = router({
  ...
  getBestRoute: protectedProcedure.input(z.object({
    amount: z.number().positive(),
    sendCurrency: z.string().length(3),
    receiveCurrency: z.string().length(3),
  })).query(async ({ input }) => {
    const routes = [
      { routeId: "route_direct", name: "Direct Transfer", gateway: "RemitFlow Core", fee: safeParseAmount((input.amount * 0.008).toFixed(2)), estimatedTime: "2-4 hours", reliability: 99.8, score: 95 },
      { routeId: "route_mojaloop", name: "Mojaloop FSPIOP", gateway: "Mojaloop Hub", fee: safeParseAmount((input.amount * 0.005).toFixed(2)), estimatedTime: "30-60 minutes", reliability: 98.9, score: 88 },
    ];
    routes.sort((a, b) => b.score - a.score);
    return { corridor: `${input.sendCurrency}-${input.receiveCurrency}`, amount: input.amount, bestRoute: routes[0] ?? null };
  }),
