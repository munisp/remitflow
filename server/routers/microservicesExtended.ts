...
let _wtDb_microservicesExtendedts: any = null;
async function _getWtDb_microservicesExtendedts() {
  if (_wtDb_microservicesExtendedts) return _wtDb_microservicesExtendedts;
  try {
    const { getDb } = await import("../db.js");
    _wtDb_microservicesExtendedts = await getDb();
    return _wtDb_microservicesExtendedts;
  } catch { return null; }
}
async function _writeThrough(table: string, key: string, value: unknown): Promise<void> {
  const db = await _getWtDb_microservicesExtendedts();
  if (!db) return;
  try {
