...
export const walletCrudRouter = router({
  ...
  add: protectedProcedure
    ...
    .mutation(async ({ ctx, input }) => {
      ...
      const existing = await db.execute(sql`SELECT id FROM wallets WHERE "userId" = ${ctx.user.id} AND currency = ${input.currency} LIMIT 1`);
      if ((existing as any[]).length > 0) {
        await db.execute(sql`UPDATE wallets SET "updatedAt" = NOW() WHERE "userId" = ${ctx.user.id} AND currency = ${input.currency}`);
      } else {
        await db.execute(sql`
