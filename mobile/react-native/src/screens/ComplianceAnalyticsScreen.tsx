...
export default function ComplianceAnalyticsScreen() {
  const navigation = useNavigation<any>();
  const { user } = useAuth();
  const [refreshing, setRefreshing] = React.useState(false);
  const { data, isLoading, refetch } = (trpc as any)?.['compliance']?.['getAnalytics']?.useQuery?.() ?? { data: null, isLoading: false, refetch: () => {} };
  const onRefresh = async () => { setRefreshing(true); await refetch?.(); setRefreshing(false); };
  const items: any[] = Array.isArray(data) ? data : (data ? [data] : []);
  return (
    <ScrollView style={styles.container} refreshControl={<RefreshControl refreshing={refreshing} onRefresh={onRefresh} />}>
      <View style={styles.header}><Text style={styles.title}>Compliance Analytics</Text></View>
      {isLoading ? <ActivityIndicator size="large" color="#7c3aed" /> : (
        <View style={styles.content}>
          {items.length === 0 ? <Text style={styles.empty}>No compliance data available</Text> : items.map((item: any, idx: number) => (
