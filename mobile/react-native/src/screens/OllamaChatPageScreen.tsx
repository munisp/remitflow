...
export default function OllamaChatPageScreen() {
  const navigation = useNavigation();
  const [search, setSearch] = useState('');
  const [refreshing, setRefreshing] = useState(false);

  const { data, isLoading, error, refetch } = trpc.ai.listConversations.useQuery(undefined, {
    retry: 2,
    staleTime: 30_000,
  });
      ...
      <ScrollView style={styles.content}
        ...
        {isLoading ? (
          ...
          <View style={styles.errorContainer}>
            <Text style={styles.errorText}>Failed to load AI Chat.</Text>
          ...
          <View style={styles.emptyContainer}>
            ...
            <Text style={styles.emptyText}>{search ? 'No results.' : 'No AI Chat yet.'}</Text>
          </View>
        ) : (
