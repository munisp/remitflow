...
export default function SendToSouthAfricaScreen() {
  const navigation = useNavigation<any>();
  const [amount, setAmount] = React.useState('');
  const { data: rates, isLoading } = trpc.fx.getRates.useQuery({ from: 'USD', to: 'ZAR' }, { onError: () => {} });
  ...
  return (
    <ScrollView style={styles.container}>
      <View style={styles.header}>
        ...
        <Text style={styles.title}>Send to South Africa</Text>
        <Text style={styles.subtitle}>Fast, secure transfers to South Africa</Text>
      </View>
      <View style={styles.card}>
        <Text style={styles.label}>Exchange Rate</Text>
        {isLoading ? <ActivityIndicator color="#6366f1" /> : (
          <Text style={styles.rate}>1 USD = {rate.toFixed(4)} ZAR</Text>
        )}
      </View>
      <View style={styles.card}>
        ...
        />
        <Text style={styles.label}>Recipient Gets (ZAR)</Text>
        <Text style={styles.converted}>{converted} Rand</Text>
      </View>
      ...
      <View style={styles.infoCard}>
        <Text style={styles.infoTitle}>Why send to South Africa?</Text>
