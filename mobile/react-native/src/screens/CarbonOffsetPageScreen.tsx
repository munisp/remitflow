...
export default function CarbonOffsetPageScreen() {
  const navigation = useNavigation();
  const [search, setSearch] = useState('');
  const [refreshing, setRefreshing] = useState(false);

  const { data, isLoading, error, refetch } = trpc.carbonOffset.listProjects.useQuery(undefined, {
    retry: 2,
    staleTime: 30_000,
  });
  ...
  return (
    <View style={styles.container}>
      <View style={styles.header}>
        ...
        <Text style={styles.title}>Carbon Projects</Text>
        <View style={{ width: 50 }} />
      </View>
      ...
      >
        {isLoading ? (
          ...
          <View style={styles.errorContainer}>
            <Text style={styles.errorText}>Failed to load Carbon Projects.</Text>
          ...
          <View style={styles.emptyContainer}>
            ...
            <Text style={styles.emptyText}>{search ? 'No results found.' : 'No Carbon Projects yet.'}</Text>
          </View>
        ) : (
