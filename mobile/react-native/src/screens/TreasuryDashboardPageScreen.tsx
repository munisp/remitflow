...
export default function TreasuryDashboardPageScreen() {
  const navigation = useNavigation<any>();
  const { user } = useAuth();
  const [refreshing, setRefreshing] = React.useState(false);
  const { data, isLoading, refetch } = (trpc as any)?.['treasury']?.['getDashboard']?.useQuery?.() ?? { data: null, isLoading: false, refetch: () => {} };
