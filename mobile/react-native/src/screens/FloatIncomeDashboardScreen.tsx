...
export default function FloatIncomeDashboardScreen() {
  const navigation = useNavigation();
  const [refreshing, setRefreshing] = React.useState(false);

  const { data, isLoading, error, refetch } = trpc.floatIncome.getDashboard.useQuery(undefined, {
    retry: 2,
    staleTime: 60_000,
  });
  ...
  return (
    <View style={styles.container}>
      <View style={styles.header}>
        ...
        <Text style={styles.title}>Float Income</Text>
        <View style={{ width: 50 }} />
      </View>
      ...
      >
        {isLoading ? (
          ...
          <View style={styles.errorContainer}>
            <Text style={styles.errorText}>Failed to load Float Income.</Text>
