...
export default function BusinessKpiDashboardScreen() {
  ...
  const loadData = async () => {
    try {
      setError(null);
      const res = await fetch(`${API_BASE}/api/trpc/business-kpi`);
  ...
  return (
    <ScrollView style={styles.container} refreshControl={<RefreshControl refreshing={refreshing} onRefresh={() => { setRefreshing(true); loadData(); }} />}>
      <Text style={styles.title}>Business KPI Dashboard</Text>
