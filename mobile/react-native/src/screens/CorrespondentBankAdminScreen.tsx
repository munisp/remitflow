...
export default function CorrespondentBankAdminScreen() {
  const navigation = useNavigation();
  const [search, setSearch] = useState('');
  const [refreshing, setRefreshing] = useState(false);

  const { data, isLoading, error, refetch } = trpc.correspondentBank.listBanks.useQuery(undefined, {
    retry: 2,
    staleTime: 30_000,
  });
  ...
  return (
    <View style={styles.container}>
      <View style={styles.header}>
        ...
        <Text style={styles.title}>Correspondent Banks</Text>
        <View style={{ width: 50 }} />
      </View>
      ...
      >
        {isLoading ? (
          ...
          <View style={styles.errorContainer}>
            <Text style={styles.errorText}>Failed to load Correspondent Banks.</Text>
          ...
          <View style={styles.emptyContainer}>
            ...
            <Text style={styles.emptyText}>{search ? 'No results found.' : 'No Correspondent Banks yet.'}</Text>
          </View>
        ) : (
