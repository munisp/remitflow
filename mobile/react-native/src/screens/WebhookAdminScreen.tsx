...
export default function WebhookAdminScreen() {
  const navigation = useNavigation<any>();
  const { user } = useAuth();
  const [refreshing, setRefreshing] = React.useState(false);
  const { data, isLoading, refetch } = (trpc as any)?.['webhooks']?.['list']?.useQuery?.() ?? { data: null, isLoading: false, refetch: () => {} };
  ...
  return (
    ...
    >
      <View style={styles.header}>
        ...
        <Text style={styles.title}>Webhook Admin</Text>
