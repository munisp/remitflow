...
export default function DailyVolumeWidgetScreen() {
  const navigation = useNavigation<any>();
  const { user } = useAuth();
  const [refreshing, setRefreshing] = React.useState(false);
  const { data, isLoading, refetch } = (trpc as any)?.['analytics']?.['getDailyVolume']?.useQuery?.() ?? { data: null, isLoading: false, refetch: () => {} };
  ...
  return (
    ...
    >
      <View style={styles.header}>
        ...
        <Text style={styles.title}>Daily Volume Widget</Text>
