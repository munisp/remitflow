...
export default function MultiCurrencyWalletV2PageScreen() {
  const navigation = useNavigation<any>();
  const { user } = useAuth();
  const [refreshing, setRefreshing] = React.useState(false);
  const { data, isLoading, refetch } = (trpc as any)?.['wallet']?.['getMultiCurrency']?.useQuery?.() ?? { data: null, isLoading: false, refetch: () => {} };
  ...
  return (
    ...
    >
      <View style={styles.header}>
        ...
        <Text style={styles.title}>Multi-Currency Wallet V2</Text>
