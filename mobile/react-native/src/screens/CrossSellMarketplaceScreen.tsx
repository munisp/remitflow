...
export default function CrossSellMarketplaceScreen() {
  const navigation = useNavigation();
  const [search, setSearch] = useState('');
  const [refreshing, setRefreshing] = useState(false);

  const { data, isLoading, error, refetch } = trpc.crossSell.listOffers.useQuery(undefined, {
    retry: 2,
    staleTime: 30_000,
  });
      ...
      >
        {isLoading ? (
          ...
          <View style={styles.errorContainer}>
            <Text style={styles.errorText}>Failed to load Marketplace Offers.</Text>
          ...
          <View style={styles.emptyContainer}>
            ...
            <Text style={styles.emptyText}>{search ? 'No results found.' : 'No Marketplace Offers yet.'}</Text>
          </View>
        ) : (
