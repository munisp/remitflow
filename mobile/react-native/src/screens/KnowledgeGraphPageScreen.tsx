...
export default function KnowledgeGraphPageScreen() {
  const navigation = useNavigation<any>();
  const { user } = useAuth();
  const [refreshing, setRefreshing] = React.useState(false);
  const { data, isLoading, refetch } = (trpc as any)?.['ai']?.['getKnowledgeGraph']?.useQuery?.() ?? { data: null, isLoading: false, refetch: () => {} };
