...
class AdminWhiteLabelScreen extends ConsumerStatefulWidget {
  const AdminWhiteLabelScreen({super.key});
  @override
  ConsumerState<AdminWhiteLabelScreen> createState() => _AdminWhiteLabelScreenState();
}

class _AdminWhiteLabelScreenState extends ConsumerState<AdminWhiteLabelScreen> {
  List<dynamic> _items = [];
  bool _loading = true;
  ...
  Future<void> _loadData() async {
    try {
      final result = await apiService.query('admin.getWhiteLabelConfig');
      if (mounted) {
        setState(() {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      appBar: AppBar(
        ...
        title: const Text('Admin White Label', style: TextStyle(color: Colors.white, fontWeight: FontWeight.bold)),
