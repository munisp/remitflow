...
class SettlementNettingScreen extends ConsumerStatefulWidget {
  const SettlementNettingScreen({super.key});
  @override
  ConsumerState<SettlementNettingScreen> createState() => _SettlementNettingScreenState();
}

class _SettlementNettingScreenState extends ConsumerState<SettlementNettingScreen> {
  List<dynamic> _items = [];
  bool _loading = true;
  ...
  Future<void> _loadData() async {
    try {
      final result = await apiService.query('settlement.getNetting');
      if (mounted) {
        setState(() {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      appBar: AppBar(
        ...
        title: const Text('Settlement Netting', style: TextStyle(color: Colors.white, fontWeight: FontWeight.bold)),
