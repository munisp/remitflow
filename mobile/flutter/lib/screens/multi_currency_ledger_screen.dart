...
class MultiCurrencyLedgerScreen extends ConsumerStatefulWidget {
  const MultiCurrencyLedgerScreen({super.key});
  @override
  ConsumerState<MultiCurrencyLedgerScreen> createState() => _MultiCurrencyLedgerScreenState();
}

class _MultiCurrencyLedgerScreenState extends ConsumerState<MultiCurrencyLedgerScreen> {
  List<dynamic> _items = [];
  bool _loading = true;
  ...
  Future<void> _loadData() async {
    try {
      final result = await apiService.query('ledger.getMultiCurrency');
      if (mounted) {
        setState(() {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      appBar: AppBar(
        ...
        title: const Text('Multi-Currency Ledger', style: TextStyle(color: Colors.white, fontWeight: FontWeight.bold)),
