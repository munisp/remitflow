...
class ChatAgentDashboardScreen extends ConsumerStatefulWidget {
  const ChatAgentDashboardScreen({super.key});
  @override
  ConsumerState<ChatAgentDashboardScreen> createState() => _ChatAgentDashboardScreenState();
}

class _ChatAgentDashboardScreenState extends ConsumerState<ChatAgentDashboardScreen> {
  List<dynamic> _items = [];
  bool _loading = true;
  ...
  Future<void> _loadData() async {
    try {
      final result = await apiService.query('chat.getAgentStats');
      if (mounted) {
        setState(() {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      appBar: AppBar(
        ...
        title: const Text('Chat Agent Dashboard', style: TextStyle(color: Colors.white, fontWeight: FontWeight.bold)),
