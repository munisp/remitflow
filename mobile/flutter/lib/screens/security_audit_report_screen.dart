...
class SecurityAuditReportScreen extends ConsumerStatefulWidget {
  const SecurityAuditReportScreen({super.key});
  @override
  ConsumerState<SecurityAuditReportScreen> createState() => _SecurityAuditReportScreenState();
}

class _SecurityAuditReportScreenState extends ConsumerState<SecurityAuditReportScreen> {
  List<dynamic> _items = [];
  bool _loading = true;
  ...
  Future<void> _loadData() async {
    try {
      final result = await apiService.query('security.getAuditReport');
      if (mounted) {
        setState(() {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      appBar: AppBar(
        ...
        title: const Text('Security Audit Report', style: TextStyle(color: Colors.white, fontWeight: FontWeight.bold)),
