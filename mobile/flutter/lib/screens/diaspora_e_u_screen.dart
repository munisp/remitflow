...
class DiasporaEUScreen extends ConsumerStatefulWidget {
  const DiasporaEUScreen({super.key});
  @override
  ConsumerState<DiasporaEUScreen> createState() => _DiasporaEUScreenState();
}

class _DiasporaEUScreenState extends ConsumerState<DiasporaEUScreen> {
  ...
  Future<void> _load() async {
    ...
    try {
      ...
      final result = await api.get('/trpc/diasporaEU.list');
              ...
              : _items.isEmpty
                  ? Center(
                      child: Column(
                        mainAxisAlignment: MainAxisAlignment.center,
                        children: [
                          Icon(Icons.euro, color: const Color(0xFF6C63FF), size: 64),
                          const SizedBox(height: 16),
                          const Text(
                            'No Diaspora EU data yet',
                        ...
                        return Card(
                          ...
                          child: ListTile(
                            leading: CircleAvatar(
                              ...
                              child: Icon(Icons.euro, color: const Color(0xFF6C63FF), size: 20),
                            ),
                            title: Text(
                              item['country']?.toString() ?? item['name']?.toString() ?? 'Item ${index + 1}',
