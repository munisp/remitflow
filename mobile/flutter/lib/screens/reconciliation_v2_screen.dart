...
class ReconciliationV2Screen extends ConsumerStatefulWidget {
  const ReconciliationV2Screen({super.key});
  @override
  ConsumerState<ReconciliationV2Screen> createState() => _ReconciliationV2ScreenState();
}

class _ReconciliationV2ScreenState extends ConsumerState<ReconciliationV2Screen> {
  List<dynamic> _items = [];
  bool _loading = true;
  ...
  Future<void> _loadData() async {
    try {
      final result = await apiService.query('ledger.getReconciliation');
      if (mounted) {
        setState(() {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      appBar: AppBar(
        ...
        title: const Text('Reconciliation V2', style: TextStyle(color: Colors.white, fontWeight: FontWeight.bold)),
