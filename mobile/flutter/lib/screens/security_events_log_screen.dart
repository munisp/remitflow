...
class SecurityEventsLogScreen extends ConsumerStatefulWidget {
  const SecurityEventsLogScreen({super.key});
  @override
  ConsumerState<SecurityEventsLogScreen> createState() => _SecurityEventsLogScreenState();
}

class _SecurityEventsLogScreenState extends ConsumerState<SecurityEventsLogScreen> {
  List<dynamic> _items = [];
  bool _loading = true;
  ...
  Future<void> _loadData() async {
    try {
      final result = await apiService.query('security.getEvents');
      if (mounted) {
        setState(() {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      appBar: AppBar(
        ...
        title: const Text('Security Events Log', style: TextStyle(color: Colors.white, fontWeight: FontWeight.bold)),
