...
class GDPRErasureScreen extends ConsumerStatefulWidget {
  const GDPRErasureScreen({super.key});
  @override
  ConsumerState<GDPRErasureScreen> createState() => _GDPRErasureScreenState();
}

class _GDPRErasureScreenState extends ConsumerState<GDPRErasureScreen> {
  ...
  Future<void> _load() async {
    ...
    try {
      ...
      final result = await api.get('/trpc/gdprErasure.list');
              ...
              : _items.isEmpty
                  ? Center(
                      child: Column(
                        mainAxisAlignment: MainAxisAlignment.center,
                        children: [
                          Icon(Icons.delete_forever, color: const Color(0xFFEF4444), size: 64),
                          const SizedBox(height: 16),
                          const Text(
                            'No GDPR Erasure data yet',
                        ...
                        return Card(
                          ...
                          child: ListTile(
                            leading: CircleAvatar(
                              ...
                              child: Icon(Icons.delete_forever, color: const Color(0xFFEF4444), size: 20),
                            ),
                            title: Text(
                              item['requestId']?.toString() ?? item['name']?.toString() ?? 'Item ${index + 1}',
