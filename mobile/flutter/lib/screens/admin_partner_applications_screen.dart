...
class AdminPartnerApplicationsScreen extends ConsumerStatefulWidget {
  const AdminPartnerApplicationsScreen({super.key});
  @override
  ConsumerState<AdminPartnerApplicationsScreen> createState() => _AdminPartnerApplicationsScreenState();
}

class _AdminPartnerApplicationsScreenState extends ConsumerState<AdminPartnerApplicationsScreen> {
  List<dynamic> _items = [];
  bool _loading = true;
  ...
  Future<void> _loadData() async {
    try {
      final result = await apiService.query('admin.getPartnerApplications');
      if (mounted) {
        setState(() {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      appBar: AppBar(
        ...
        title: const Text('Admin Partner Applications', style: TextStyle(color: Colors.white, fontWeight: FontWeight.bold)),
