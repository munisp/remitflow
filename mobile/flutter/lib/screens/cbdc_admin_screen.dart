...
class _CBDCAdminScreenState extends ConsumerState<CBDCAdminScreen> {
  ...
  Future<void> _loadData() async {
    try {
      final result = await apiService.query('cbdc.getStatus');
      if (mounted) {
        setState(() {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      appBar: AppBar(
        ...
        title: const Text('CBDC Admin', style: TextStyle(color: Colors.white, fontWeight: FontWeight.bold)),
          ...
          : _items.isEmpty
              ? Center(
                  child: Column(
                    ...
                    children: [
                      Icon(Icons.currency_bitcoin, size: 64, color: const Color(0xFF64748B)),
