...
class ChargebackManagerScreen extends ConsumerStatefulWidget {
  const ChargebackManagerScreen({super.key});
  @override
  ConsumerState<ChargebackManagerScreen> createState() => _ChargebackManagerScreenState();
}

class _ChargebackManagerScreenState extends ConsumerState<ChargebackManagerScreen> {
  List<dynamic> _items = [];
  bool _loading = true;
  ...
  Future<void> _loadData() async {
    try {
      final result = await apiService.query('chargebacks.list');
      if (mounted) {
        setState(() {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      appBar: AppBar(
        ...
        title: const Text('Chargeback Manager', style: TextStyle(color: Colors.white, fontWeight: FontWeight.bold)),
