...
class CrossBorderComplianceScreen extends ConsumerStatefulWidget {
  const CrossBorderComplianceScreen({super.key});
  @override
  ConsumerState<CrossBorderComplianceScreen> createState() => _CrossBorderComplianceScreenState();
}

class _CrossBorderComplianceScreenState extends ConsumerState<CrossBorderComplianceScreen> {
  List<dynamic> _items = [];
  bool _loading = true;
  ...
  Future<void> _loadData() async {
    try {
      final result = await apiService.query('compliance.getCrossBorder');
      if (mounted) {
        setState(() {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      appBar: AppBar(
        ...
        title: const Text('Cross-Border Compliance', style: TextStyle(color: Colors.white, fontWeight: FontWeight.bold)),
