...
class FraudDetectionV2PageScreen extends ConsumerStatefulWidget {
  const FraudDetectionV2PageScreen({super.key});
  @override
  ConsumerState<FraudDetectionV2PageScreen> createState() => _FraudDetectionV2PageScreenState();
}

class _FraudDetectionV2PageScreenState extends ConsumerState<FraudDetectionV2PageScreen> {
  ...
  Future<void> _load() async {
    ...
    try {
      ...
      final result = await api.get('/trpc/fraudDetection.list');
              ...
              : _items.isEmpty
                  ? Center(
                      child: Column(
                        mainAxisAlignment: MainAxisAlignment.center,
                        children: [
                          Icon(Icons.security, color: const Color(0xFFEF4444), size: 64),
                          const SizedBox(height: 16),
                          const Text(
                            'No Fraud Detection data yet',
                        ...
                        return Card(
                          ...
                          child: ListTile(
                            leading: CircleAvatar(
                              ...
                              child: Icon(Icons.security, color: const Color(0xFFEF4444), size: 20),
                            ),
                            title: Text(
                              item['transactionId']?.toString() ?? item['name']?.toString() ?? 'Item ${index + 1}',
