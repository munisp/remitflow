...
class BatchPaymentsScreen extends ConsumerStatefulWidget {
  const BatchPaymentsScreen({super.key});
  @override
  ConsumerState<BatchPaymentsScreen> createState() => _BatchPaymentsScreenState();
}

class _BatchPaymentsScreenState extends ConsumerState<BatchPaymentsScreen> {
  ...
  Future<void> _load() async {
    try {
      ...
      final result = await api.get('/trpc/batchPayments.list');
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      appBar: AppBar(
        ...
        title: const Text('Batch Payments', style: TextStyle(color: Color(0xFFE2E8F0), fontWeight: FontWeight.w700)),
          ...
          : _error != null
              ...
              : _items.isEmpty
                  ? Center(child: Column(mainAxisAlignment: MainAxisAlignment.center, children: [
                      ...
                      const SizedBox(height: 12),
                      const Text('No Batch Payments yet', style: TextStyle(color: Color(0xFF9CA3AF), fontSize: 16)),
                    ]))
                  : RefreshIndicator(
                      onRefresh: _load,
