...
class SanctionsScreeningPageScreen extends ConsumerStatefulWidget {
  const SanctionsScreeningPageScreen({super.key});
  @override
  ConsumerState<SanctionsScreeningPageScreen> createState() => _SanctionsScreeningPageScreenState();
}

class _SanctionsScreeningPageScreenState extends ConsumerState<SanctionsScreeningPageScreen> {
  ...
  Future<void> _load() async {
    ...
    try {
      ...
      final result = await api.get('/trpc/sanctionsScreening.list');
              ...
              : _items.isEmpty
                  ? Center(
                      child: Column(
                        mainAxisAlignment: MainAxisAlignment.center,
                        children: [
                          Icon(Icons.block, color: const Color(0xFFEF4444), size: 64),
                          const SizedBox(height: 16),
                          const Text(
                            'No Sanctions Screening data yet',
                        ...
                        return Card(
                          ...
                          child: ListTile(
                            leading: CircleAvatar(
                              ...
                              child: Icon(Icons.block, color: const Color(0xFFEF4444), size: 20),
                            ),
                            title: Text(
                              item['entity']?.toString() ?? item['name']?.toString() ?? 'Item ${index + 1}',
