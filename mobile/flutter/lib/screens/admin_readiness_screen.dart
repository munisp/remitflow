...
class AdminReadinessScreen extends ConsumerStatefulWidget {
  const AdminReadinessScreen({super.key});
  @override
  ConsumerState<AdminReadinessScreen> createState() => _AdminReadinessScreenState();
}

class _AdminReadinessScreenState extends ConsumerState<AdminReadinessScreen> {
  List<dynamic> _items = [];
  bool _loading = true;
  ...
  Future<void> _loadData() async {
    try {
      final result = await apiService.query('admin.getReadiness');
      if (mounted) {
        setState(() {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      appBar: AppBar(
        ...
        title: const Text('Admin Readiness', style: TextStyle(color: Colors.white, fontWeight: FontWeight.bold)),
