...
class RegulatoryReportingScreen extends ConsumerStatefulWidget {
  const RegulatoryReportingScreen({super.key});
  @override
  ConsumerState<RegulatoryReportingScreen> createState() => _RegulatoryReportingScreenState();
}

class _RegulatoryReportingScreenState extends ConsumerState<RegulatoryReportingScreen> {
  List<dynamic> _items = [];
  bool _loading = true;
  ...
  Future<void> _loadData() async {
    try {
      final result = await apiService.query('compliance.getRegulatoryReports');
      if (mounted) {
        setState(() {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      appBar: AppBar(
        ...
        title: const Text('Regulatory Reporting', style: TextStyle(color: Colors.white, fontWeight: FontWeight.bold)),
