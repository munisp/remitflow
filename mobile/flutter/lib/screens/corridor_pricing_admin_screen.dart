...
class CorridorPricingAdminScreen extends ConsumerStatefulWidget {
  const CorridorPricingAdminScreen({super.key});
  @override
  ConsumerState<CorridorPricingAdminScreen> createState() => _CorridorPricingAdminScreenState();
}
class _CorridorPricingAdminScreenState extends ConsumerState<CorridorPricingAdminScreen> {
  List<dynamic> _items = [];
  bool _loading = true;
  @override
  void initState() {
  ...
  Future<void> _loadData() async {
    try {
      final result = await apiService.query('corridorPricing.list');
      if (mounted) {
        setState(() {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      appBar: AppBar(
        ...
        title: const Text('Corridor Pricing', style: TextStyle(color: Colors.white, fontWeight: FontWeight.bold)),
          ...
          : _items.isEmpty
              ? Center(
                  child: Column(
                    ...
                    children: [
                      Icon(Icons.swap_horiz, size: 64, color: const Color(0xFF64748B)),
