...
class AdminDigitalAgreementsScreen extends ConsumerStatefulWidget {
  const AdminDigitalAgreementsScreen({super.key});
  @override
  ConsumerState<AdminDigitalAgreementsScreen> createState() => _AdminDigitalAgreementsScreenState();
}

class _AdminDigitalAgreementsScreenState extends ConsumerState<AdminDigitalAgreementsScreen> {
  List<dynamic> _items = [];
  bool _loading = true;
  ...
  Future<void> _loadData() async {
    try {
      final result = await apiService.query('admin.getAgreements');
      if (mounted) {
        setState(() {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      appBar: AppBar(
        ...
        title: const Text('Admin Digital Agreements', style: TextStyle(color: Colors.white, fontWeight: FontWeight.bold)),
