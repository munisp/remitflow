...
class TalentBridgeScreen extends ConsumerStatefulWidget {
  const TalentBridgeScreen({super.key});
  @override
  ConsumerState<TalentBridgeScreen> createState() => _TalentBridgeScreenState();
}

class _TalentBridgeScreenState extends ConsumerState<TalentBridgeScreen> {
  List<dynamic> _items = [];
  bool _loading = true;
  ...
  Future<void> _loadData() async {
    try {
      final result = await _api.query('talentBridge.getOpportunities');
      if (mounted) {
        setState(() {
          _items = List<dynamic>.from(result['opportunities'] ?? result['data'] ?? result['result'] ?? []);
          _loading = false;
        });
      }
    } catch (e) {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      appBar: AppBar(
        ...
        title: const Text('Talent Bridge', style: TextStyle(color: Colors.white, fontWeight: FontWeight.bold)),
