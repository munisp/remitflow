...
class BeneficiaryManagerScreen extends ConsumerStatefulWidget {
  const BeneficiaryManagerScreen({super.key});
  @override
  ConsumerState<BeneficiaryManagerScreen> createState() => _BeneficiaryManagerScreenState();
}

class _BeneficiaryManagerScreenState extends ConsumerState<BeneficiaryManagerScreen> {
  List<dynamic> _items = [];
  bool _loading = true;
  ...
  Future<void> _loadData() async {
    try {
      final result = await apiService.query('beneficiaries.list');
      if (mounted) {
        setState(() {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      appBar: AppBar(
        ...
        title: const Text('Beneficiary Manager', style: TextStyle(color: Colors.white, fontWeight: FontWeight.bold)),
