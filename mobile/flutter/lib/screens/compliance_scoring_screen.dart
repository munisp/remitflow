...
class ComplianceScoringScreen extends ConsumerStatefulWidget {
  const ComplianceScoringScreen({super.key});
  @override
  ConsumerState<ComplianceScoringScreen> createState() => _ComplianceScoringScreenState();
}

class _ComplianceScoringScreenState extends ConsumerState<ComplianceScoringScreen> {
  List<dynamic> _items = [];
  bool _loading = true;
  ...
  Future<void> _loadData() async {
    try {
      final result = await apiService.query('compliance.getScores');
      if (mounted) {
        setState(() {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      appBar: AppBar(
        ...
        title: const Text('Compliance Scoring', style: TextStyle(color: Colors.white, fontWeight: FontWeight.bold)),
