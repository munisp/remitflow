...
class ARTAgentScreen extends ConsumerStatefulWidget {
  const ARTAgentScreen({super.key});
  @override
  ConsumerState<ARTAgentScreen> createState() => _ARTAgentScreenState();
}

class _ARTAgentScreenState extends ConsumerState<ARTAgentScreen> {
  List<dynamic> _items = [];
  bool _loading = true;
  ...
  Future<void> _loadData() async {
    try {
      final result = await apiService.query('compliance.getARTStatus');
      if (mounted) {
        setState(() {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      appBar: AppBar(
        ...
        title: const Text('ART Agent', style: TextStyle(color: Colors.white, fontWeight: FontWeight.bold)),
