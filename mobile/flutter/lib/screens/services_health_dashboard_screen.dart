...
class ServicesHealthDashboardScreen extends ConsumerStatefulWidget {
  const ServicesHealthDashboardScreen({super.key});
  @override
  ConsumerState<ServicesHealthDashboardScreen> createState() => _ServicesHealthDashboardScreenState();
}
class _ServicesHealthDashboardScreenState extends ConsumerState<ServicesHealthDashboardScreen> {
  List<dynamic> _items = [];
  bool _loading = true;
  @override
  void initState() {
  ...
  Future<void> _loadData() async {
    try {
      final result = await apiService.query('servicesHealth.getAll');
      if (mounted) {
        setState(() {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      appBar: AppBar(
        ...
        title: const Text('Services Health', style: TextStyle(color: Colors.white, fontWeight: FontWeight.bold)),
          ...
          : _items.isEmpty
              ? Center(
                  child: Column(
                    ...
                    children: [
                      Icon(Icons.health_and_safety, size: 64, color: const Color(0xFF64748B)),
