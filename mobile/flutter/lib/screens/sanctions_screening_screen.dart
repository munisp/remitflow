...
class SanctionsScreeningScreen extends ConsumerStatefulWidget {
  const SanctionsScreeningScreen({super.key});
  @override
  ConsumerState<SanctionsScreeningScreen> createState() => _SanctionsScreeningScreenState();
}

class _SanctionsScreeningScreenState extends ConsumerState<SanctionsScreeningScreen> {
  List<dynamic> _items = [];
  bool _loading = true;
  ...
  Future<void> _loadData() async {
    try {
      final result = await apiService.query('compliance.getSanctions');
      if (mounted) {
        setState(() {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      appBar: AppBar(
        ...
        title: const Text('Sanctions Screening', style: TextStyle(color: Colors.white, fontWeight: FontWeight.bold)),
