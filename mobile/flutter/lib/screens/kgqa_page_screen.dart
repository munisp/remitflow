...
class KGQAScreen extends ConsumerStatefulWidget {
  const KGQAScreen({super.key});
  @override
  ConsumerState<KGQAScreen> createState() => _KGQAScreenState();
}
class _KGQAScreenState extends ConsumerState<KGQAScreen> {
  List<dynamic> _items = [];
  bool _loading = true;
  @override
  void initState() {
  ...
  Future<void> _loadData() async {
    try {
      final result = await apiService.query('kgqa.query');
      if (mounted) {
        setState(() {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      appBar: AppBar(
        ...
        title: const Text('Knowledge Graph QA', style: TextStyle(color: Colors.white, fontWeight: FontWeight.bold)),
          ...
          : _items.isEmpty
              ? Center(
                  child: Column(
                    ...
                    children: [
                      Icon(Icons.hub, size: 64, color: const Color(0xFF64748B)),
