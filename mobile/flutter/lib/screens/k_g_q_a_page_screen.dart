...
class KGQAPageScreen extends ConsumerStatefulWidget {
  const KGQAPageScreen({super.key});
  @override
  ConsumerState<KGQAPageScreen> createState() => _KGQAPageScreenState();
}

class _KGQAPageScreenState extends ConsumerState<KGQAPageScreen> {
  ...
  Future<void> _load() async {
    ...
    try {
      ...
      final result = await api.get('/trpc/kgqa.list');
              ...
              : _items.isEmpty
                  ? Center(
                      child: Column(
                        mainAxisAlignment: MainAxisAlignment.center,
                        children: [
                          Icon(Icons.question_answer, color: const Color(0xFF6C63FF), size: 64),
                          const SizedBox(height: 16),
                          const Text(
                            'No Knowledge Graph Q&A data yet',
                        ...
                        return Card(
                          ...
                          child: ListTile(
                            leading: CircleAvatar(
                              ...
                              child: Icon(Icons.question_answer, color: const Color(0xFF6C63FF), size: 20),
                            ),
                            title: Text(
                              item['question']?.toString() ?? item['name']?.toString() ?? 'Item ${index + 1}',
