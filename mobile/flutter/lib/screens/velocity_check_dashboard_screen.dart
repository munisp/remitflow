...
class VelocityCheckDashboardScreen extends ConsumerStatefulWidget {
  const VelocityCheckDashboardScreen({super.key});
  @override
  ConsumerState<VelocityCheckDashboardScreen> createState() => _VelocityCheckDashboardScreenState();
}

class _VelocityCheckDashboardScreenState extends ConsumerState<VelocityCheckDashboardScreen> {
  List<dynamic> _items = [];
  bool _loading = true;
  ...
  Future<void> _loadData() async {
    try {
      final result = await apiService.query('compliance.getVelocityChecks');
      if (mounted) {
        setState(() {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      appBar: AppBar(
        ...
        title: const Text('Velocity Check Dashboard', style: TextStyle(color: Colors.white, fontWeight: FontWeight.bold)),
