...
class FXHedgingPageScreen extends ConsumerStatefulWidget {
  const FXHedgingPageScreen({super.key});
  @override
  ConsumerState<FXHedgingPageScreen> createState() => _FXHedgingPageScreenState();
}

class _FXHedgingPageScreenState extends ConsumerState<FXHedgingPageScreen> {
  ...
  Future<void> _load() async {
    ...
    try {
      ...
      final result = await api.get('/trpc/fxHedging.list');
              ...
              : _items.isEmpty
                  ? Center(
                      child: Column(
                        mainAxisAlignment: MainAxisAlignment.center,
                        children: [
                          Icon(Icons.trending_up, color: const Color(0xFF10B981), size: 64),
                          const SizedBox(height: 16),
                          const Text(
                            'No FX Hedging data yet',
                        ...
                        return Card(
                          ...
                          child: ListTile(
                            leading: CircleAvatar(
                              ...
                              child: Icon(Icons.trending_up, color: const Color(0xFF10B981), size: 20),
                            ),
                            title: Text(
                              item['instrument']?.toString() ?? item['name']?.toString() ?? 'Item ${index + 1}',
