...
class _SLAMonitorScreenState extends ConsumerState<SLAMonitorScreen> {
  ...
  Future<void> _load() async {
    ...
    try {
      ...
      final result = await api.get('/trpc/slaMonitor.list');
              ...
              : _items.isEmpty
                  ? Center(
                      child: Column(
                        mainAxisAlignment: MainAxisAlignment.center,
                        children: [
                          Icon(Icons.monitor_heart, color: const Color(0xFF10B981), size: 64),
                          const SizedBox(height: 16),
                          const Text(
                            'No SLA Monitor data yet',
                        ...
                        return Card(
                          ...
                          child: ListTile(
                            leading: CircleAvatar(
                              ...
                              child: Icon(Icons.monitor_heart, color: const Color(0xFF10B981), size: 20),
                            ),
                            title: Text(
                              item['service']?.toString() ?? item['name']?.toString() ?? 'Item ${index + 1}',
