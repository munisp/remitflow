...
class StripeReceiptsScreen extends ConsumerStatefulWidget {
  const StripeReceiptsScreen({super.key});
  @override
  ConsumerState<StripeReceiptsScreen> createState() => _StripeReceiptsScreenState();
}

class _StripeReceiptsScreenState extends ConsumerState<StripeReceiptsScreen> {
  List<dynamic> _items = [];
  bool _loading = true;
  ...
  Future<void> _loadData() async {
    try {
      final result = await apiService.query('stripe.getReceipts');
      if (mounted) {
        setState(() {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      appBar: AppBar(
        ...
        title: const Text('Stripe Receipts', style: TextStyle(color: Colors.white, fontWeight: FontWeight.bold)),
