...
class FXOptionsPricingScreen extends ConsumerStatefulWidget {
  const FXOptionsPricingScreen({super.key});
  @override
  ConsumerState<FXOptionsPricingScreen> createState() => _FXOptionsPricingScreenState();
}

class _FXOptionsPricingScreenState extends ConsumerState<FXOptionsPricingScreen> {
  List<dynamic> _items = [];
  bool _loading = true;
  ...
  Future<void> _loadData() async {
    try {
      final result = await apiService.query('fx.getOptions');
      if (mounted) {
        setState(() {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      appBar: AppBar(
        ...
        title: const Text('FX Options Pricing', style: TextStyle(color: Colors.white, fontWeight: FontWeight.bold)),
