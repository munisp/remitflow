...
class BulkUserActionsScreen extends ConsumerStatefulWidget {
  const BulkUserActionsScreen({super.key});
  @override
  ConsumerState<BulkUserActionsScreen> createState() => _BulkUserActionsScreenState();
}

class _BulkUserActionsScreenState extends ConsumerState<BulkUserActionsScreen> {
  List<dynamic> _items = [];
  bool _loading = true;
  ...
  Future<void> _loadData() async {
    try {
      final result = await apiService.query('admin.getBulkUserActions');
      if (mounted) {
        setState(() {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      appBar: AppBar(
        ...
        title: const Text('Bulk User Actions', style: TextStyle(color: Colors.white, fontWeight: FontWeight.bold)),
