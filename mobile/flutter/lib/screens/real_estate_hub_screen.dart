...
class RealEstateHubScreen extends ConsumerStatefulWidget {
  const RealEstateHubScreen({super.key});
  @override
  ConsumerState<RealEstateHubScreen> createState() => _RealEstateHubScreenState();
}

class _RealEstateHubScreenState extends ConsumerState<RealEstateHubScreen> {
  List<dynamic> _items = [];
  bool _loading = true;
  ...
  Future<void> _loadData() async {
    try {
      final result = await _api.query('realEstate.getListings');
      if (mounted) {
        setState(() {
          _items = List<dynamic>.from(result['listings'] ?? result['data'] ?? result['result'] ?? []);
          _loading = false;
        });
      }
    } catch (e) {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      appBar: AppBar(
        ...
        title: const Text('Real Estate Hub', style: TextStyle(color: Colors.white, fontWeight: FontWeight.bold)),
