...
class LakehousePageScreen extends ConsumerStatefulWidget {
  const LakehousePageScreen({super.key});
  @override
  ConsumerState<LakehousePageScreen> createState() => _LakehousePageScreenState();
}

class _LakehousePageScreenState extends ConsumerState<LakehousePageScreen> {
  ...
  Future<void> _load() async {
    ...
    try {
      ...
      final result = await api.get('/trpc/lakehouse.list');
              ...
              : _items.isEmpty
                  ? Center(
                      child: Column(
                        mainAxisAlignment: MainAxisAlignment.center,
                        children: [
                          Icon(Icons.storage, color: const Color(0xFF6C63FF), size: 64),
                          const SizedBox(height: 16),
                          const Text(
                            'No Data Lakehouse data yet',
                        ...
                        return Card(
                          ...
                          child: ListTile(
                            leading: CircleAvatar(
                              ...
                              child: Icon(Icons.storage, color: const Color(0xFF6C63FF), size: 20),
                            ),
                            title: Text(
                              item['dataset']?.toString() ?? item['name']?.toString() ?? 'Item ${index + 1}',
