...
class KafkaDashboardScreen extends ConsumerStatefulWidget {
  const KafkaDashboardScreen({super.key});
  @override
  ConsumerState<KafkaDashboardScreen> createState() => _KafkaDashboardScreenState();
}

class _KafkaDashboardScreenState extends ConsumerState<KafkaDashboardScreen> {
  List<dynamic> _items = [];
  bool _loading = true;
  ...
  Future<void> _loadData() async {
    try {
      final result = await apiService.query('admin.getKafkaMetrics');
      if (mounted) {
        setState(() {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      appBar: AppBar(
        ...
        title: const Text('Kafka Dashboard', style: TextStyle(color: Colors.white, fontWeight: FontWeight.bold)),
