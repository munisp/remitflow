...
class LiquidityMonitorScreen extends ConsumerStatefulWidget {
  const LiquidityMonitorScreen({super.key});
  @override
  ConsumerState<LiquidityMonitorScreen> createState() => _LiquidityMonitorScreenState();
}

class _LiquidityMonitorScreenState extends ConsumerState<LiquidityMonitorScreen> {
  List<dynamic> _items = [];
  bool _loading = true;
  ...
  Future<void> _loadData() async {
    try {
      final result = await apiService.query('treasury.getLiquidity');
      if (mounted) {
        setState(() {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      appBar: AppBar(
        ...
        title: const Text('Liquidity Monitor', style: TextStyle(color: Colors.white, fontWeight: FontWeight.bold)),
