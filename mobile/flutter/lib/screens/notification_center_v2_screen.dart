...
class NotificationCenterV2Screen extends ConsumerStatefulWidget {
  const NotificationCenterV2Screen({super.key});
  @override
  ConsumerState<NotificationCenterV2Screen> createState() => _NotificationCenterV2ScreenState();
}

class _NotificationCenterV2ScreenState extends ConsumerState<NotificationCenterV2Screen> {
  List<dynamic> _items = [];
  bool _loading = true;
  ...
  Future<void> _loadData() async {
    try {
      final result = await apiService.query('notifications.list');
      if (mounted) {
        setState(() {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      appBar: AppBar(
        ...
        title: const Text('Notification Center V2', style: TextStyle(color: Colors.white, fontWeight: FontWeight.bold)),
