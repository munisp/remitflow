...
class CheckoutSdkScreen extends ConsumerStatefulWidget {
  const CheckoutSdkScreen({super.key});
  @override
  ConsumerState<CheckoutSdkScreen> createState() => _CheckoutSdkScreenState();
}

class _CheckoutSdkScreenState extends ConsumerState<CheckoutSdkScreen> {
  ...
  Future<void> _load() async {
    try {
      ...
      final result = await api.get('/trpc/checkout.apiKeys');
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      appBar: AppBar(
        ...
        title: const Text('Checkout SDK', style: TextStyle(color: Color(0xFFE2E8F0), fontWeight: FontWeight.w700)),
          ...
          : _error != null
              ...
              : _items.isEmpty
                  ? Center(child: Column(mainAxisAlignment: MainAxisAlignment.center, children: [
                      ...
                      const SizedBox(height: 12),
                      const Text('No Checkout SDK yet', style: TextStyle(color: Color(0xFF9CA3AF), fontSize: 16)),
                    ]))
                  : RefreshIndicator(
                      onRefresh: _load,
