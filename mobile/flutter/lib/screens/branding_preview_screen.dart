...
class BrandingPreviewScreen extends ConsumerStatefulWidget {
  const BrandingPreviewScreen({super.key});
  @override
  ConsumerState<BrandingPreviewScreen> createState() => _BrandingPreviewScreenState();
}

class _BrandingPreviewScreenState extends ConsumerState<BrandingPreviewScreen> {
  List<dynamic> _items = [];
  bool _loading = true;
  ...
  Future<void> _loadData() async {
    try {
      final result = await _api.query('whiteLabel.getBranding');
      if (mounted) {
        setState(() {
          _items = List<dynamic>.from(result['config'] ?? result['data'] ?? result['result'] ?? []);
          _loading = false;
        });
      }
    } catch (e) {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      appBar: AppBar(
        ...
        title: const Text('Branding Preview', style: TextStyle(color: Colors.white, fontWeight: FontWeight.bold)),
