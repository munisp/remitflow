...
class SandboxScenariosScreen extends ConsumerStatefulWidget {
  const SandboxScenariosScreen({super.key});
  @override
  ConsumerState<SandboxScenariosScreen> createState() => _SandboxScenariosScreenState();
}

class _SandboxScenariosScreenState extends ConsumerState<SandboxScenariosScreen> {
  List<dynamic> _items = [];
  bool _loading = true;
  ...
  Future<void> _loadData() async {
    try {
      final result = await apiService.query('dev.getSandboxScenarios');
      if (mounted) {
        setState(() {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      appBar: AppBar(
        ...
        title: const Text('Sandbox Scenarios', style: TextStyle(color: Colors.white, fontWeight: FontWeight.bold)),
