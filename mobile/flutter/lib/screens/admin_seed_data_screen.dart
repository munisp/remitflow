...
class AdminSeedDataScreen extends ConsumerStatefulWidget {
  const AdminSeedDataScreen({super.key});
  @override
  ConsumerState<AdminSeedDataScreen> createState() => _AdminSeedDataScreenState();
}

class _AdminSeedDataScreenState extends ConsumerState<AdminSeedDataScreen> {
  List<dynamic> _items = [];
  bool _loading = true;
  ...
  Future<void> _loadData() async {
    try {
      final result = await apiService.query('admin.getSeedStatus');
      if (mounted) {
        setState(() {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      appBar: AppBar(
        ...
        title: const Text('Admin Seed Data', style: TextStyle(color: Colors.white, fontWeight: FontWeight.bold)),
