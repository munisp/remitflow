...
class AdminStripeTestScreen extends ConsumerStatefulWidget {
  const AdminStripeTestScreen({super.key});
  @override
  ConsumerState<AdminStripeTestScreen> createState() => _AdminStripeTestScreenState();
}

class _AdminStripeTestScreenState extends ConsumerState<AdminStripeTestScreen> {
  List<dynamic> _items = [];
  bool _loading = true;
  ...
  Future<void> _loadData() async {
    try {
      final result = await apiService.query('admin.getStripeStatus');
      if (mounted) {
        setState(() {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      appBar: AppBar(
        ...
        title: const Text('Admin Stripe Test', style: TextStyle(color: Colors.white, fontWeight: FontWeight.bold)),
