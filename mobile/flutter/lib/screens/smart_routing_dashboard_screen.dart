...
class SmartRoutingDashboardScreen extends ConsumerStatefulWidget {
  const SmartRoutingDashboardScreen({super.key});
  @override
  ConsumerState<SmartRoutingDashboardScreen> createState() => _SmartRoutingDashboardScreenState();
}

class _SmartRoutingDashboardScreenState extends ConsumerState<SmartRoutingDashboardScreen> {
  List<dynamic> _items = [];
  bool _loading = true;
  ...
  Future<void> _loadData() async {
    try {
      final result = await apiService.query('routing.getDashboard');
      if (mounted) {
        setState(() {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      appBar: AppBar(
        ...
        title: const Text('Smart Routing Dashboard', style: TextStyle(color: Colors.white, fontWeight: FontWeight.bold)),
