...
class LoyaltyRewardsV2Screen extends ConsumerStatefulWidget {
  const LoyaltyRewardsV2Screen({super.key});
  @override
  ConsumerState<LoyaltyRewardsV2Screen> createState() => _LoyaltyRewardsV2ScreenState();
}

class _LoyaltyRewardsV2ScreenState extends ConsumerState<LoyaltyRewardsV2Screen> {
  List<dynamic> _items = [];
  bool _loading = true;
  ...
  Future<void> _loadData() async {
    try {
      final result = await apiService.query('loyalty.getRewards');
      if (mounted) {
        setState(() {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      appBar: AppBar(
        ...
        title: const Text('Loyalty Rewards V2', style: TextStyle(color: Colors.white, fontWeight: FontWeight.bold)),
