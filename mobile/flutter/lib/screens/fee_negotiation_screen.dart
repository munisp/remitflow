...
class FeeNegotiationScreen extends ConsumerStatefulWidget {
  const FeeNegotiationScreen({super.key});
  @override
  ConsumerState<FeeNegotiationScreen> createState() => _FeeNegotiationScreenState();
}

class _FeeNegotiationScreenState extends ConsumerState<FeeNegotiationScreen> {
  List<dynamic> _items = [];
  bool _loading = true;
  ...
  Future<void> _loadData() async {
    try {
      final result = await apiService.query('fees.getNegotiations');
      if (mounted) {
        setState(() {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      appBar: AppBar(
        ...
        title: const Text('Fee Negotiation', style: TextStyle(color: Colors.white, fontWeight: FontWeight.bold)),
