...
class _BnplScreenState extends ConsumerState<BnplScreen> {
  ...
  Future<void> _load() async {
    ...
    try {
      ...
      final result = await api.get('/trpc/bnpl.list');
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      appBar: AppBar(
        ...
        title: const Text('BNPL', style: TextStyle(color: Color(0xFFE2E8F0), fontWeight: FontWeight.w700)),
      ...
      body: _isLoading
          ...
          : _error != null
              ...
              : _items.isEmpty
                  ? const Center(child: Text('No BNPL plans yet', style: TextStyle(color: Color(0xFF94A3B8))))
                  : ListView.builder(
                      ...
                      itemBuilder: (context, index) {
                        ...
                        return Card(
                          ...
                          child: ListTile(
                            title: Text(item['planName']?.toString() ?? item['merchant']?.toString() ?? 'BNPL Plan ${index + 1}',
                                style: const TextStyle(color: Color(0xFFE2E8F0), fontWeight: FontWeight.w600)),
                            subtitle: Text(
                                'Status: ${item['status']?.toString() ?? 'N/A'}'
                                '${item['amount'] != null ? ' - Amount: \$${(item['amount'] as num).toStringAsFixed(2)}' : ''}'
