...
class StripePaymentHistoryScreen extends ConsumerStatefulWidget {
  const StripePaymentHistoryScreen({super.key});
  @override
  ConsumerState<StripePaymentHistoryScreen> createState() => _StripePaymentHistoryScreenState();
}

class _StripePaymentHistoryScreenState extends ConsumerState<StripePaymentHistoryScreen> {
  List<dynamic> _items = [];
  bool _loading = true;
  ...
  Future<void> _loadData() async {
    try {
      final result = await apiService.query('stripe.getPaymentHistory');
      if (mounted) {
        setState(() {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      appBar: AppBar(
        ...
        title: const Text('Stripe Payment History', style: TextStyle(color: Colors.white, fontWeight: FontWeight.bold)),
