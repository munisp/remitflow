...
class DisputeManagementScreen extends ConsumerStatefulWidget {
  const DisputeManagementScreen({super.key});
  @override
  ConsumerState<DisputeManagementScreen> createState() => _DisputeManagementScreenState();
}

class _DisputeManagementScreenState extends ConsumerState<DisputeManagementScreen> {
  List<dynamic> _items = [];
  bool _loading = true;
  ...
  Future<void> _loadData() async {
    try {
      final result = await apiService.query('disputes.list');
      if (mounted) {
        setState(() {
