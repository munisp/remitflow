...
class GDPRDataScreen extends ConsumerStatefulWidget {
  const GDPRDataScreen({super.key});
  @override
  ConsumerState<GDPRDataScreen> createState() => _GDPRDataScreenState();
}

class _GDPRDataScreenState extends ConsumerState<GDPRDataScreen> {
  ...
  Future<void> _load() async {
    ...
    try {
      ...
      final result = await api.get('/trpc/gdpr.list');
              ...
              : _items.isEmpty
                  ? Center(
                      child: Column(
                        mainAxisAlignment: MainAxisAlignment.center,
                        children: [
                          Icon(Icons.privacy_tip, color: const Color(0xFF6C63FF), size: 64),
                          const SizedBox(height: 16),
                          const Text(
                            'No GDPR Data data yet',
                        ...
                        return Card(
                          ...
                          child: ListTile(
                            leading: CircleAvatar(
                              ...
                              child: Icon(Icons.privacy_tip, color: const Color(0xFF6C63FF), size: 20),
                            ),
                            title: Text(
                              item['subject']?.toString() ?? item['name']?.toString() ?? 'Item ${index + 1}',
