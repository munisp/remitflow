...
class LiquidityStressTestScreen extends ConsumerStatefulWidget {
  const LiquidityStressTestScreen({super.key});
  @override
  ConsumerState<LiquidityStressTestScreen> createState() => _LiquidityStressTestScreenState();
}

class _LiquidityStressTestScreenState extends ConsumerState<LiquidityStressTestScreen> {
  List<dynamic> _items = [];
  bool _loading = true;
  ...
  Future<void> _loadData() async {
    try {
      final result = await apiService.query('treasury.getStressTests');
      if (mounted) {
        setState(() {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      appBar: AppBar(
        ...
        title: const Text('Liquidity Stress Test', style: TextStyle(color: Colors.white, fontWeight: FontWeight.bold)),
