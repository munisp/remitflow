...
class VirtualAccountScreen extends ConsumerStatefulWidget {
  const VirtualAccountScreen({super.key});
  @override
  ConsumerState<VirtualAccountScreen> createState() => _VirtualAccountScreenState();
}

class _VirtualAccountScreenState extends ConsumerState<VirtualAccountScreen> {
  ...
  Future<void> _load() async {
    try {
      ...
      final result = await api.get('/trpc/virtualAccounts.list');
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      appBar: AppBar(
        ...
        title: const Text('Virtual Accounts', style: TextStyle(color: Color(0xFFE2E8F0), fontWeight: FontWeight.w700)),
          ...
          : _error != null
              ...
              : _items.isEmpty
                  ? Center(child: Column(mainAxisAlignment: MainAxisAlignment.center, children: [
                      ...
                      const SizedBox(height: 12),
                      const Text('No Virtual Accounts yet', style: TextStyle(color: Color(0xFF9CA3AF), fontSize: 16)),
                    ]))
                  : RefreshIndicator(
                      onRefresh: _load,
