...
class PromoCodesAdminScreen extends ConsumerStatefulWidget {
  const PromoCodesAdminScreen({super.key});
  @override
  ConsumerState<PromoCodesAdminScreen> createState() => _PromoCodesAdminScreenState();
}

class _PromoCodesAdminScreenState extends ConsumerState<PromoCodesAdminScreen> {
  List<dynamic> _items = [];
  bool _loading = true;
  ...
  Future<void> _loadData() async {
    try {
      final result = await apiService.query('admin.getPromoCodes');
      if (mounted) {
        setState(() {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      appBar: AppBar(
        ...
        title: const Text('Promo Codes Admin', style: TextStyle(color: Colors.white, fontWeight: FontWeight.bold)),
