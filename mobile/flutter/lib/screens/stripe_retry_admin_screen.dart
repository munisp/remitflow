...
class StripeRetryAdminScreen extends ConsumerStatefulWidget {
  const StripeRetryAdminScreen({super.key});
  @override
  ConsumerState<StripeRetryAdminScreen> createState() => _StripeRetryAdminScreenState();
}

class _StripeRetryAdminScreenState extends ConsumerState<StripeRetryAdminScreen> {
  List<dynamic> _items = [];
  bool _loading = true;
  ...
  Future<void> _loadData() async {
    try {
      final result = await apiService.query('stripe.getRetryQueue');
      if (mounted) {
        setState(() {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      appBar: AppBar(
        ...
        title: const Text('Stripe Retry Admin', style: TextStyle(color: Colors.white, fontWeight: FontWeight.bold)),
