...
class DocumentVaultRenewalScreen extends ConsumerStatefulWidget {
  const DocumentVaultRenewalScreen({super.key});
  @override
  ConsumerState<DocumentVaultRenewalScreen> createState() => _DocumentVaultRenewalScreenState();
}

class _DocumentVaultRenewalScreenState extends ConsumerState<DocumentVaultRenewalScreen> {
  List<dynamic> _items = [];
  bool _loading = true;
  ...
  Future<void> _loadData() async {
    try {
      final result = await apiService.query('kyc.getDocuments');
      if (mounted) {
        setState(() {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      appBar: AppBar(
        ...
        title: const Text('Document Vault Renewal', style: TextStyle(color: Colors.white, fontWeight: FontWeight.bold)),
