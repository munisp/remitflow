...
class FCAComplianceScreen extends ConsumerStatefulWidget {
  const FCAComplianceScreen({super.key});
  @override
  ConsumerState<FCAComplianceScreen> createState() => _FCAComplianceScreenState();
}

class _FCAComplianceScreenState extends ConsumerState<FCAComplianceScreen> {
  ...
  Future<void> _load() async {
    ...
    try {
      ...
      final result = await api.get('/trpc/fcaCompliance.list');
              ...
              : _items.isEmpty
                  ? Center(
                      child: Column(
                        mainAxisAlignment: MainAxisAlignment.center,
                        children: [
                          Icon(Icons.gavel, color: const Color(0xFFEF4444), size: 64),
                          const SizedBox(height: 16),
                          const Text(
                            'No FCA Compliance data yet',
                        ...
                        return Card(
                          ...
                          child: ListTile(
                            leading: CircleAvatar(
                              ...
                              child: Icon(Icons.gavel, color: const Color(0xFFEF4444), size: 20),
                            ),
                            title: Text(
                              item['requirement']?.toString() ?? item['name']?.toString() ?? 'Item ${index + 1}',
