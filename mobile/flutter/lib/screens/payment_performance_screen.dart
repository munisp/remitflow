...
class PaymentPerformanceScreen extends ConsumerStatefulWidget {
  const PaymentPerformanceScreen({super.key});
  @override
  ConsumerState<PaymentPerformanceScreen> createState() => _PaymentPerformanceScreenState();
}

class _PaymentPerformanceScreenState extends ConsumerState<PaymentPerformanceScreen> {
  List<dynamic> _items = [];
  bool _loading = true;
  ...
  Future<void> _loadData() async {
    try {
      final result = await apiService.query('analytics.getPaymentPerformance');
      if (mounted) {
        setState(() {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      appBar: AppBar(
        ...
        title: const Text('Payment Performance', style: TextStyle(color: Colors.white, fontWeight: FontWeight.bold)),
