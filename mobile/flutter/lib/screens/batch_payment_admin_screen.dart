...
class BatchPaymentAdminScreen extends ConsumerStatefulWidget {
  const BatchPaymentAdminScreen({super.key});
  @override
  ConsumerState<BatchPaymentAdminScreen> createState() => _BatchPaymentAdminScreenState();
}

class _BatchPaymentAdminScreenState extends ConsumerState<BatchPaymentAdminScreen> {
  List<dynamic> _items = [];
  bool _loading = true;
  ...
  Future<void> _loadData() async {
    try {
      final result = await apiService.query('batch.getAdminBatches');
      if (mounted) {
        setState(() {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      appBar: AppBar(
        ...
        title: const Text('Batch Payment Admin', style: TextStyle(color: Colors.white, fontWeight: FontWeight.bold)),
