...
class AMLBatchEngineScreen extends ConsumerStatefulWidget {
  const AMLBatchEngineScreen({super.key});
  @override
  ConsumerState<AMLBatchEngineScreen> createState() => _AMLBatchEngineScreenState();
}

class _AMLBatchEngineScreenState extends ConsumerState<AMLBatchEngineScreen> {
  List<dynamic> _items = [];
  bool _loading = true;
  ...
  Future<void> _loadData() async {
    try {
      final result = await apiService.query('compliance.getAMLBatches');
      if (mounted) {
        setState(() {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      appBar: AppBar(
        ...
        title: const Text('AML Batch Engine', style: TextStyle(color: Colors.white, fontWeight: FontWeight.bold)),
