...
class PartnerPayoutsV2Screen extends ConsumerStatefulWidget {
  const PartnerPayoutsV2Screen({super.key});
  @override
  ConsumerState<PartnerPayoutsV2Screen> createState() => _PartnerPayoutsV2ScreenState();
}

class _PartnerPayoutsV2ScreenState extends ConsumerState<PartnerPayoutsV2Screen> {
  List<dynamic> _items = [];
  bool _loading = true;
  ...
  Future<void> _loadData() async {
    try {
      final result = await apiService.query('partner.getPayouts');
      if (mounted) {
        setState(() {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      appBar: AppBar(
        ...
        title: const Text('Partner Payouts V2', style: TextStyle(color: Colors.white, fontWeight: FontWeight.bold)),
