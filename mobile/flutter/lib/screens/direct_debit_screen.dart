...
class DirectDebitScreen extends ConsumerStatefulWidget {
  const DirectDebitScreen({super.key});
  @override
  ConsumerState<DirectDebitScreen> createState() => _DirectDebitScreenState();
}

class _DirectDebitScreenState extends ConsumerState<DirectDebitScreen> {
  ...
  Future<void> _load() async {
    try {
      ...
      final result = await api.get('/trpc/directDebit.mandates');
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      appBar: AppBar(
        ...
        title: const Text('Direct Debit', style: TextStyle(color: Color(0xFFE2E8F0), fontWeight: FontWeight.w700)),
          ...
          : _error != null
              ...
              : _items.isEmpty
                  ? Center(child: Column(mainAxisAlignment: MainAxisAlignment.center, children: [
                      ...
                      const SizedBox(height: 12),
                      const Text('No Direct Debit yet', style: TextStyle(color: Color(0xFF9CA3AF), fontSize: 16)),
                    ]))
                  : RefreshIndicator(
                      onRefresh: _load,
