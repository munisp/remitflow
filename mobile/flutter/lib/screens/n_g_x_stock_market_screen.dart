...
class _NGXStockMarketScreenState extends ConsumerState<NGXStockMarketScreen> {
  ...
  Future<void> _load() async {
    ...
    try {
      ...
      final result = await api.get('/trpc/ngxStockMarket.list');
              ...
              : _items.isEmpty
                  ? Center(
                      child: Column(
                        mainAxisAlignment: MainAxisAlignment.center,
                        children: [
                          Icon(Icons.show_chart, color: const Color(0xFF10B981), size: 64),
                          const SizedBox(height: 16),
                          const Text(
                            'No NGX Stock Market data yet',
                        ...
                        return Card(
                          ...
                          child: ListTile(
                            leading: CircleAvatar(
                              ...
                              child: Icon(Icons.show_chart, color: const Color(0xFF10B981), size: 20),
                            ),
                            title: Text(
                              item['symbol']?.toString() ?? item['name']?.toString() ?? 'Item ${index + 1}',
