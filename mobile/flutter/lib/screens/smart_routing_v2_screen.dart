...
class SmartRoutingV2Screen extends ConsumerStatefulWidget {
  const SmartRoutingV2Screen({super.key});
  @override
  ConsumerState<SmartRoutingV2Screen> createState() => _SmartRoutingV2ScreenState();
}

class _SmartRoutingV2ScreenState extends ConsumerState<SmartRoutingV2Screen> {
  List<dynamic> _items = [];
  bool _loading = true;
  ...
  Future<void> _loadData() async {
    try {
      final result = await apiService.query('routing.getRoutes');
      if (mounted) {
        setState(() {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      appBar: AppBar(
        ...
        title: const Text('Smart Routing V2', style: TextStyle(color: Colors.white, fontWeight: FontWeight.bold)),
