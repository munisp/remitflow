...
class MultiCurrencyWalletV2Screen extends ConsumerStatefulWidget {
  const MultiCurrencyWalletV2Screen({super.key});
  @override
  ConsumerState<MultiCurrencyWalletV2Screen> createState() => _MultiCurrencyWalletV2ScreenState();
}

class _MultiCurrencyWalletV2ScreenState extends ConsumerState<MultiCurrencyWalletV2Screen> {
  List<dynamic> _items = [];
  bool _loading = true;
  ...
  Future<void> _loadData() async {
    try {
      final result = await apiService.query('wallet.getMultiCurrency');
      if (mounted) {
        setState(() {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      appBar: AppBar(
        ...
        title: const Text('Multi-Currency Wallet V2', style: TextStyle(color: Colors.white, fontWeight: FontWeight.bold)),
