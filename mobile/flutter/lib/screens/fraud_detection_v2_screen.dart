...
class FraudDetectionV2Screen extends ConsumerStatefulWidget {
  const FraudDetectionV2Screen({super.key});
  @override
  ConsumerState<FraudDetectionV2Screen> createState() => _FraudDetectionV2ScreenState();
}

class _FraudDetectionV2ScreenState extends ConsumerState<FraudDetectionV2Screen> {
  List<dynamic> _items = [];
  bool _loading = true;
  ...
  Future<void> _loadData() async {
    try {
      final result = await apiService.query('fraud.getAlerts');
      if (mounted) {
        setState(() {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      appBar: AppBar(
        ...
        title: const Text('Fraud Detection V2', style: TextStyle(color: Colors.white, fontWeight: FontWeight.bold)),
