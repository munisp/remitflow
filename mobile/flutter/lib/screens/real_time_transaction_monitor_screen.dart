...
class RealTimeTransactionMonitorScreen extends ConsumerStatefulWidget {
  const RealTimeTransactionMonitorScreen({super.key});
  @override
  ConsumerState<RealTimeTransactionMonitorScreen> createState() => _RealTimeTransactionMonitorScreenState();
}

class _RealTimeTransactionMonitorScreenState extends ConsumerState<RealTimeTransactionMonitorScreen> {
  List<dynamic> _items = [];
  bool _loading = true;
  ...
  Future<void> _loadData() async {
    try {
      final result = await apiService.query('transactions.list');
      if (mounted) {
        setState(() {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      appBar: AppBar(
        ...
        title: const Text('Real-Time Monitor', style: TextStyle(color: Colors.white, fontWeight: FontWeight.bold)),
