...
class DailyVolumeWidgetScreen extends ConsumerStatefulWidget {
  const DailyVolumeWidgetScreen({super.key});
  @override
  ConsumerState<DailyVolumeWidgetScreen> createState() => _DailyVolumeWidgetScreenState();
}

class _DailyVolumeWidgetScreenState extends ConsumerState<DailyVolumeWidgetScreen> {
  List<dynamic> _items = [];
  bool _loading = true;
  ...
  Future<void> _loadData() async {
    try {
      final result = await apiService.query('analytics.getDailyVolume');
      if (mounted) {
        setState(() {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      appBar: AppBar(
        ...
        title: const Text('Daily Volume Widget', style: TextStyle(color: Colors.white, fontWeight: FontWeight.bold)),
