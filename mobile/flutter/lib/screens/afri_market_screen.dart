...
class AfriMarketScreen extends ConsumerStatefulWidget {
  const AfriMarketScreen({super.key});
  @override
  ConsumerState<AfriMarketScreen> createState() => _AfriMarketScreenState();
}

class _AfriMarketScreenState extends ConsumerState<AfriMarketScreen> {
  List<dynamic> _items = [];
  bool _loading = true;
  ...
  Future<void> _loadData() async {
    try {
      final result = await _api.query('afriMarket.list');
      if (mounted) {
        setState(() {
          _items = List<dynamic>.from(result['items'] ?? result['data'] ?? result['result'] ?? []);
          _loading = false;
        });
      }
    } catch (e) {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      appBar: AppBar(
        ...
        title: const Text('AfriMarket', style: TextStyle(color: Colors.white, fontWeight: FontWeight.bold)),
