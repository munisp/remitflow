...
class AuditTrailV2Screen extends ConsumerStatefulWidget {
  const AuditTrailV2Screen({super.key});
  @override
  ConsumerState<AuditTrailV2Screen> createState() => _AuditTrailV2ScreenState();
}

class _AuditTrailV2ScreenState extends ConsumerState<AuditTrailV2Screen> {
  List<dynamic> _items = [];
  bool _loading = true;
  ...
  Future<void> _loadData() async {
    try {
      final result = await apiService.query('audit.getTrail');
      if (mounted) {
        setState(() {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      appBar: AppBar(
        ...
        title: const Text('Audit Trail V2', style: TextStyle(color: Colors.white, fontWeight: FontWeight.bold)),
