...
class ComplianceReportingScreen extends ConsumerStatefulWidget {
  const ComplianceReportingScreen({super.key});
  @override
  ConsumerState<ComplianceReportingScreen> createState() => _ComplianceReportingScreenState();
}

class _ComplianceReportingScreenState extends ConsumerState<ComplianceReportingScreen> {
  List<dynamic> _items = [];
  bool _loading = true;
  ...
  Future<void> _loadData() async {
    try {
      final result = await apiService.query('compliance.getReports');
      if (mounted) {
        setState(() {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      appBar: AppBar(
        ...
        title: const Text('Compliance Reporting', style: TextStyle(color: Colors.white, fontWeight: FontWeight.bold)),
