...
class ScheduledTransfersV2Screen extends ConsumerStatefulWidget {
  const ScheduledTransfersV2Screen({super.key});
  @override
  ConsumerState<ScheduledTransfersV2Screen> createState() => _ScheduledTransfersV2ScreenState();
}

class _ScheduledTransfersV2ScreenState extends ConsumerState<ScheduledTransfersV2Screen> {
  List<dynamic> _items = [];
  bool _loading = true;
  ...
  Future<void> _loadData() async {
    try {
      final result = await apiService.query('scheduled.list');
      if (mounted) {
        setState(() {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      appBar: AppBar(
        ...
        title: const Text('Scheduled Transfers V2', style: TextStyle(color: Colors.white, fontWeight: FontWeight.bold)),
