...
class FeeRulesCRUDV2Screen extends ConsumerStatefulWidget {
  const FeeRulesCRUDV2Screen({super.key});
  @override
  ConsumerState<FeeRulesCRUDV2Screen> createState() => _FeeRulesCRUDV2ScreenState();
}
class _FeeRulesCRUDV2ScreenState extends ConsumerState<FeeRulesCRUDV2Screen> {
  List<dynamic> _items = [];
  bool _loading = true;
  @override
  void initState() {
  ...
  Future<void> _loadData() async {
    try {
      final result = await apiService.query('feeEngine.getRules');
      if (mounted) {
        setState(() {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      appBar: AppBar(
        ...
        title: const Text('Fee Rules V2', style: TextStyle(color: Colors.white, fontWeight: FontWeight.bold)),
          ...
          : _items.isEmpty
              ? Center(
                  child: Column(
                    ...
                    children: [
                      Icon(Icons.price_change, size: 64, color: const Color(0xFF64748B)),
