...
class MultiHopRoutingScreen extends ConsumerStatefulWidget {
  const MultiHopRoutingScreen({super.key});
  @override
  ConsumerState<MultiHopRoutingScreen> createState() => _MultiHopRoutingScreenState();
}

class _MultiHopRoutingScreenState extends ConsumerState<MultiHopRoutingScreen> {
  List<dynamic> _items = [];
  bool _loading = true;
  ...
  Future<void> _loadData() async {
    try {
      final result = await apiService.query('routing.getMultiHop');
      if (mounted) {
        setState(() {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      appBar: AppBar(
        ...
        title: const Text('Multi-Hop Routing', style: TextStyle(color: Colors.white, fontWeight: FontWeight.bold)),
