...
class RateAlertHistoryScreen extends ConsumerStatefulWidget {
  const RateAlertHistoryScreen({super.key});
  @override
  ConsumerState<RateAlertHistoryScreen> createState() => _RateAlertHistoryScreenState();
}

class _RateAlertHistoryScreenState extends ConsumerState<RateAlertHistoryScreen> {
  List<dynamic> _items = [];
  bool _loading = true;
  ...
  Future<void> _loadData() async {
    try {
      final result = await apiService.query('fx.getAlertHistory');
      if (mounted) {
        setState(() {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      appBar: AppBar(
        ...
        title: const Text('Rate Alert History', style: TextStyle(color: Colors.white, fontWeight: FontWeight.bold)),
