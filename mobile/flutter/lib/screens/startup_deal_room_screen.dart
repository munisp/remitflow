...
class StartupDealRoomScreen extends ConsumerStatefulWidget {
  const StartupDealRoomScreen({super.key});
  @override
  ConsumerState<StartupDealRoomScreen> createState() => _StartupDealRoomScreenState();
}

class _StartupDealRoomScreenState extends ConsumerState<StartupDealRoomScreen> {
  List<dynamic> _items = [];
  bool _loading = true;
  ...
  Future<void> _loadData() async {
    try {
      final result = await _api.query('startupDealRoom.getDeals');
      if (mounted) {
        setState(() {
          _items = List<dynamic>.from(result['deals'] ?? result['data'] ?? result['result'] ?? []);
          _loading = false;
        });
      }
    } catch (e) {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      appBar: AppBar(
        ...
        title: const Text('Startup Deal Room', style: TextStyle(color: Colors.white, fontWeight: FontWeight.bold)),
