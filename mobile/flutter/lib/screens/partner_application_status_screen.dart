...
class PartnerApplicationStatusScreen extends ConsumerStatefulWidget {
  const PartnerApplicationStatusScreen({super.key});
  @override
  ConsumerState<PartnerApplicationStatusScreen> createState() => _PartnerApplicationStatusScreenState();
}

class _PartnerApplicationStatusScreenState extends ConsumerState<PartnerApplicationStatusScreen> {
  List<dynamic> _items = [];
  bool _loading = true;
  ...
  Future<void> _loadData() async {
    try {
      final result = await apiService.query('partner.getApplicationStatus');
      if (mounted) {
        setState(() {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      appBar: AppBar(
        ...
        title: const Text('Partner Application Status', style: TextStyle(color: Colors.white, fontWeight: FontWeight.bold)),
