...
class WebhookRetryScreen extends ConsumerStatefulWidget {
  const WebhookRetryScreen({super.key});
  @override
  ConsumerState<WebhookRetryScreen> createState() => _WebhookRetryScreenState();
}

class _WebhookRetryScreenState extends ConsumerState<WebhookRetryScreen> {
  List<dynamic> _items = [];
  bool _loading = true;
  ...
  Future<void> _loadData() async {
    try {
      final result = await apiService.query('webhooks.getRetryQueue');
      if (mounted) {
        setState(() {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      appBar: AppBar(
        ...
        title: const Text('Webhook Retry', style: TextStyle(color: Colors.white, fontWeight: FontWeight.bold)),
