...
class BulkPaymentsV2Screen extends ConsumerStatefulWidget {
  const BulkPaymentsV2Screen({super.key});
  @override
  ConsumerState<BulkPaymentsV2Screen> createState() => _BulkPaymentsV2ScreenState();
}

class _BulkPaymentsV2ScreenState extends ConsumerState<BulkPaymentsV2Screen> {
  List<dynamic> _items = [];
  bool _loading = true;
  ...
  Future<void> _loadData() async {
    try {
      final result = await apiService.query('batch.getBulkPayments');
      if (mounted) {
        setState(() {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      appBar: AppBar(
        ...
        title: const Text('Bulk Payments V2', style: TextStyle(color: Colors.white, fontWeight: FontWeight.bold)),
