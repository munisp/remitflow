...
class ComplianceEmailConfigScreen extends ConsumerStatefulWidget {
  const ComplianceEmailConfigScreen({super.key});
  @override
  ConsumerState<ComplianceEmailConfigScreen> createState() => _ComplianceEmailConfigScreenState();
}

class _ComplianceEmailConfigScreenState extends ConsumerState<ComplianceEmailConfigScreen> {
  List<dynamic> _items = [];
  bool _loading = true;
  ...
  Future<void> _loadData() async {
    try {
      final result = await apiService.query('compliance.getEmailConfig');
      if (mounted) {
        setState(() {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      appBar: AppBar(
        ...
        title: const Text('Compliance Email Config', style: TextStyle(color: Colors.white, fontWeight: FontWeight.bold)),
