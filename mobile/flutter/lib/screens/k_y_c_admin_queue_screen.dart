...
class KYCAdminQueueScreen extends ConsumerStatefulWidget {
  const KYCAdminQueueScreen({super.key});
  @override
  ConsumerState<KYCAdminQueueScreen> createState() => _KYCAdminQueueScreenState();
}

class _KYCAdminQueueScreenState extends ConsumerState<KYCAdminQueueScreen> {
  ...
  Future<void> _load() async {
    ...
    try {
      ...
      final result = await api.get('/trpc/kycAdmin.list');
              ...
              : _items.isEmpty
                  ? Center(
                      child: Column(
                        mainAxisAlignment: MainAxisAlignment.center,
                        children: [
                          Icon(Icons.admin_panel_settings, color: const Color(0xFFEF4444), size: 64),
                          const SizedBox(height: 16),
                          const Text(
                            'No KYC Admin Queue data yet',
                        ...
                        return Card(
                          ...
                          child: ListTile(
                            leading: CircleAvatar(
                              ...
                              child: Icon(Icons.admin_panel_settings, color: const Color(0xFFEF4444), size: 20),
                            ),
                            title: Text(
                              item['userId']?.toString() ?? item['name']?.toString() ?? 'Item ${index + 1}',
