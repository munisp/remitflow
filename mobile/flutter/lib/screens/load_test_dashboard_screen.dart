...
class LoadTestDashboardScreen extends ConsumerStatefulWidget {
  const LoadTestDashboardScreen({super.key});
  @override
  ConsumerState<LoadTestDashboardScreen> createState() => _LoadTestDashboardScreenState();
}

class _LoadTestDashboardScreenState extends ConsumerState<LoadTestDashboardScreen> {
  List<dynamic> _items = [];
  bool _loading = true;
  ...
  Future<void> _loadData() async {
    try {
      final result = await apiService.query('admin.getLoadTestResults');
      if (mounted) {
        setState(() {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      appBar: AppBar(
        ...
        title: const Text('Load Test Dashboard', style: TextStyle(color: Colors.white, fontWeight: FontWeight.bold)),
