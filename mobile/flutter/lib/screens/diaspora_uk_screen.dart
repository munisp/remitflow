...
class DiasporaUKScreen extends ConsumerStatefulWidget {
  const DiasporaUKScreen({super.key});
  @override
  ConsumerState<DiasporaUKScreen> createState() => _DiasporaUKScreenState();
}

class _DiasporaUKScreenState extends ConsumerState<DiasporaUKScreen> {
  List<dynamic> _items = [];
  bool _loading = true;
  ...
  Future<void> _loadData() async {
    try {
      final result = await _api.query('diaspora.getCorridorInfo');
      if (mounted) {
        setState(() {
          _items = List<dynamic>.from(result['info'] ?? result['data'] ?? result['result'] ?? []);
          _loading = false;
        });
      }
    } catch (e) {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      appBar: AppBar(
        ...
        title: const Text('Diaspora UK', style: TextStyle(color: Colors.white, fontWeight: FontWeight.bold)),
