...
class TransferLimitsV2Screen extends ConsumerStatefulWidget {
  const TransferLimitsV2Screen({super.key});
  @override
  ConsumerState<TransferLimitsV2Screen> createState() => _TransferLimitsV2ScreenState();
}

class _TransferLimitsV2ScreenState extends ConsumerState<TransferLimitsV2Screen> {
  List<dynamic> _items = [];
  bool _loading = true;
  ...
  Future<void> _loadData() async {
    try {
      final result = await apiService.query('transfers.getLimits');
      if (mounted) {
        setState(() {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      appBar: AppBar(
        ...
        title: const Text('Transfer Limits V2', style: TextStyle(color: Colors.white, fontWeight: FontWeight.bold)),
