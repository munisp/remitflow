...
class CocoIndexScreen extends ConsumerStatefulWidget {
  const CocoIndexScreen({super.key});
  @override
  ConsumerState<CocoIndexScreen> createState() => _CocoIndexScreenState();
}

class _CocoIndexScreenState extends ConsumerState<CocoIndexScreen> {
  List<dynamic> _items = [];
  bool _loading = true;
  ...
  Future<void> _loadData() async {
    try {
      final result = await _api.query('cocoIndex.getData');
      if (mounted) {
        setState(() {
          _items = List<dynamic>.from(result['data'] ?? result['data'] ?? result['result'] ?? []);
          _loading = false;
        });
      }
    } catch (e) {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      appBar: AppBar(
        ...
        title: const Text('COCO Index', style: TextStyle(color: Colors.white, fontWeight: FontWeight.bold)),
