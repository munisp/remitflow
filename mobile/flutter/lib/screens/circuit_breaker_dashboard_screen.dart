...
class CircuitBreakerDashboardScreen extends ConsumerStatefulWidget {
  const CircuitBreakerDashboardScreen({super.key});
  @override
  ConsumerState<CircuitBreakerDashboardScreen> createState() => _CircuitBreakerDashboardScreenState();
}

class _CircuitBreakerDashboardScreenState extends ConsumerState<CircuitBreakerDashboardScreen> {
  List<dynamic> _items = [];
  bool _loading = true;
  ...
  Future<void> _loadData() async {
    try {
      final result = await apiService.query('system.getCircuitBreakers');
      if (mounted) {
        setState(() {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      appBar: AppBar(
        ...
        title: const Text('Circuit Breaker Dashboard', style: TextStyle(color: Colors.white, fontWeight: FontWeight.bold)),
