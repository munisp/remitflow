...
class WiseTransferScreen extends ConsumerStatefulWidget {
  const WiseTransferScreen({super.key});
  @override
  ConsumerState<WiseTransferScreen> createState() => _WiseTransferScreenState();
}

class _WiseTransferScreenState extends ConsumerState<WiseTransferScreen> {
  List<dynamic> _items = [];
  bool _loading = true;
  ...
  Future<void> _loadData() async {
    try {
      final result = await apiService.query('payments.getWiseStatus');
      if (mounted) {
        setState(() {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      appBar: AppBar(
        ...
        title: const Text('Wise Transfer', style: TextStyle(color: Colors.white, fontWeight: FontWeight.bold)),
