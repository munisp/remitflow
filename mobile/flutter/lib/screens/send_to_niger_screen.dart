...
class SendToNigerScreen extends ConsumerStatefulWidget {
  const SendToNigerScreen({super.key});
  @override
  ConsumerState<SendToNigerScreen> createState() => _SendToNigerScreenState();
}

class _SendToNigerScreenState extends ConsumerState<SendToNigerScreen> {
  ...
  Future<void> _load() async {
    ...
    try {
      ...
      final result = await api.get('/trpc/sendMoney.list');
              ...
              : _items.isEmpty
                  ? Center(
                      child: Column(
                        mainAxisAlignment: MainAxisAlignment.center,
                        children: [
                          Icon(Icons.send, color: const Color(0xFF10B981), size: 64),
                          const SizedBox(height: 16),
                          const Text(
                            'No Send to Niger data yet',
                        ...
                        return Card(
                          ...
                          child: ListTile(
                            leading: CircleAvatar(
                              ...
                              child: Icon(Icons.send, color: const Color(0xFF10B981), size: 20),
                            ),
                            title: Text(
